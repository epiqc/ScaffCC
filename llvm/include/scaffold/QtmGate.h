//===----------------- QtmGate.h ------------------------------------------===//
// This file implements the Scaffold Quantum Gate Class
//
...
char QtmInst::ID = 0;
static RegisterPass<QtmInst> X("QtmInst",
                               "Get gate and operands for a Qtm Inst");
...
bool QtmInst::checkIfQbitOrCbit(Type *varType) {
  if (varType->isPointerTy()) {
    ...
    if (elementType->isIntegerTy(16) ||
        elementType->isIntegerTy(1)) { // qbit* or cbit*
      return true;
    }
  } else if (varType->isIntegerTy(16) ||
             varType->isIntegerTy(1)) { // qbit or cbit
    return true;
  }
  return false;
}
...
bool QtmInst::backtraceOperand(Value *opd, int opOrIndex) {
  if (opOrIndex == 0) // backtrace for operand
  {
    ...
    if (isa<GetElementPtrInst>(opd)) {
      ...
      // considering last operand as desired index for qubit. This may not be
      // true for multi-dimensional qubit arrays
      if (ConstantInt *CI =
        ...
        if (tmpDepQbit[0].isQbit && !(tmpDepQbit[0].isPtr)) {
          ...
          // last operand as desired index for qubit. This may not be true for
          // multi-dimensional qubit arrays
