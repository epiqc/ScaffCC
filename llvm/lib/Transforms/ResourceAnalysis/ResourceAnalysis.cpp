//===-- ResourceAnalysis.cpp - Resource Analysis for Scaffold -------------===//
//
...
public:
  ...
  Func(string n)
      : name(n), self_bits(0), total_bits_min(0), total_bits_max(0),
...
struct ScaffoldResourceAnalysis : public CallGraphSCCPass {
  ...
  virtual bool runOnSCC(CallGraphSCC &SCC) {
    ...
    for (CallGraphNode *Node : SCC) {
      ...
      // Insert call graph
      for (CallGraphNode::iterator node_itr = Node->begin();
           node_itr != Node->end(); node_itr++) {
        if (node_itr->second->getFunction())
          fnode->calls.insert(
              make_pair(node_itr->second->getFunction()->getName(),
  ...
  void dump(Func *func, int tab, int ref) {
    ...
    if (ref)
      cout << "(" << ref << ")";
    std::cout << func->name << " (SB=" << func->self_bits
              << ", TBMin=" << func->total_bits_min
              << ", TBMax=" << func->total_bits_max
              << ", SOp=" << func->self_ops << ", TOp=" << func->total_ops
              << ")" << std::endl;
...
static RegisterPass<ScaffoldResourceAnalysis> X("sra",
                                                "Scaffold resource analysis",
