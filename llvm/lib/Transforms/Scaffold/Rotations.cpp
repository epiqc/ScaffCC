...
struct Rotations : public ModulePass {
  ...
  struct RotationVisitor : public InstVisitor<RotationVisitor> {
    ...
    void visitCallInst(CallInst &I) {
      ...
      double Angle =
          cast<ConstantFP>(I.getArgOperand(1))->getValueAPF().convertToDouble();
      ...
      // Sanitize strings
      for (std::string::iterator iter = FuncName.begin(); iter < FuncName.end();
           iter++) {
        switch (*iter) {
        case '-':
          FuncName.replace(iter, iter + 1, "n");
          break;
        case '+':
          FuncName.erase(iter);
          iter--;
          break;
        case '.':
          FuncName.replace(iter, iter + 1, "_");
          break;
        case '"':
          FuncName.erase(iter);
          iter--;
      ...
      FunctionType *FuncType = FunctionType::get(
          ...
          ArrayRef<Type *>(Type::getInt16Ty(M->getContext())), false);
      ...
      if (!DR) {
        ...
        BasicBlock *BB = BasicBlock::Create(M->getContext(), "", DR, 0);
        ...
        // (the decomposed string is given in the reverse order that ops must be
        // applied)
        for (int i = circuit.length() - 2; i >= 0; i--) {
          ...
          newCallInst =
              CallInst::Create(gate, ArrayRef<Value *>(DR->arg_begin()),
                               // Insert at front
                               // "", BB->front());
                               // Insert at end
                               "", BB);
...
char Rotations::ID = 0;
static RegisterPass<Rotations> X("Rotations", "Rotation Decomposition", false,
                                 false);
