...
struct CriticalResourceCount : public ModulePass {
  ...
  vector<unsigned long long int> curr_parallel_ts; // vector of current
                                                   // timesteps that are
                                                   // parallel; used for
                                                   // comparing functions
...
static RegisterPass<CriticalResourceCount> X("CriticalResourceCount",
                                             "Critical Resource Counter Pass");
...
void CriticalResourceCount::print_vector_TSGates(Function *F) {
  map<string, TSInfo>::iterator fp =
      funcParallelFactor.find(F->getName().str());
  errs() << "Printing function map \n";
  for (vector<TSParGateInfo>::iterator vit = (*fp).second.gates.begin();
       vit != (*fp).second.gates.end(); ++vit) {
    for (int i = 0; i < NUM_QGATES; i++) {
...
void CriticalResourceCount::print_currParallelFunc() {
  ...
  for (vector<string>::iterator vit = currParallelFunc.begin();
       vit != currParallelFunc.end(); ++vit) {
    errs() << (*vit) << " ";
  }
  errs() << "\n";
}

void CriticalResourceCount::reset_current_ts_info(
    Function *F) { // current timestep info
...
void CriticalResourceCount::print_max_critical_info(
    MaxTSInfo ts) { // max timestep info
...
void CriticalResourceCount::print_TSInfo(TSInfo ts) {
  for (vector<TSParGateInfo>::iterator printIt = ts.gates.begin();
       printIt != ts.gates.end(); ++printIt) {
...
void CriticalResourceCount::update_max_critical_info(
    Function *F) { // max timestep info
  ...
  if (currParallelFunc.size() > 1) {
    ...
    for (vector<string>::iterator vit = currParallelFunc.begin();
      ...
      if (debugCritDataPath)
        errs() << "Timesteps of function: " << (*vit) << " are "
               << (*tsIter).second.timesteps << "\n";
      ...
      if (vsize < fnsize) {
        for (unsigned int j = 0; j < vsize; j++) {
          for (int k = 0; k < NUM_QGATES; k++)
            sum_info.gates[j].parallel_gates[k] +=
                (*tsIter).second.gates[j].parallel_gates[k];
        }
    ...
    if (F->getName() != "main") { // do not save for main
      map<string, TSInfo>::iterator fnIter =
          funcParallelFactor.find(F->getName().str());
      // append sum_info.gates to fnIter.second.gates
      (*fnIter).second.gates.insert((*fnIter).second.gates.end(),
                                    sum_info.gates.begin(),
                                    sum_info.gates.end());
    }

    // compare maxParallelFactor with the data in sum_info
    for (vector<TSParGateInfo>::iterator gateIter = sum_info.gates.begin();
         gateIter != sum_info.gates.end(); ++gateIter) {
      for (int i = 0; i < NUM_QGATES; i++) {
    ...
    // directly compare maxParallelFactor with the timesteps in function in
    // currParallelFunc
    ...
    if (debugCritDataPath)
      errs() << "Timesteps of function: " << fStr << " are "
             << (*tsIter).second.timesteps << "\n";
    ...
    if (F->getName() != "main") { // do not save for main
      ...
      // append sum_info.gates to fnIter.second.gates
      (*fnIter).second.gates.insert((*fnIter).second.gates.end(),
                                    (*tsIter).second.gates.begin(),
                                    (*tsIter).second.gates.end());
    }
    ...
    for (int i = 0; i < NUM_QGATES; i++) {
      if ((*maxIter).second.parallel_gates[i] >
          maxParallelFactor.parallel_gates[i])
        maxParallelFactor.parallel_gates[i] =
            (*maxIter).second.parallel_gates[i];
    }
...
void CriticalResourceCount::calc_critical_time(Function *F, qGate qg) {
  ...
  for (vector<qArgInfo>::iterator vit = currTimeStep.begin();
       (vit != currTimeStep.end()) && (!is_dependency); ++vit) {
    for (int i = 0; i < qg.numArgs; i++) {
      if ((*vit).name == qg.args[i].name) // var name matches
      {
        if ((*vit).index == -1 ||
            qg.args[i].index ==
                -1) { // argument is a ptr, so pessimistic checking
          is_dependency = true;
          break;
        } else {
...
void CriticalResourceCount::copy_max_parallel_factor(Function *F) {
  map<string, MaxTSInfo>::iterator fparIter =
      funcMaxParallelFactor.find(F->getName().str());
...
void CriticalResourceCount::CountCriticalFunctionResources(Function *F) {
  ...
  map<string, TSInfo>::iterator fparIter =
      funcParallelFactor.find(F->getName().str());
  (*fparIter).second.timesteps = maxParallelFactor.timesteps;
