...
static RegisterPass<GenQASMLoops>
    X("gen-qasm-with-loops",
      "Generate QASM output code with repeat statements"); // spatil: should be
                                                           // Z or X??
...
void GenQASMLoops::analyzeCallInst(Function *F, Instruction *pInst) {
  if (CallInst *CI = dyn_cast<CallInst>(pInst)) {
    ...
    if (((CI->getCalledFunction()->getName()).find("qasmRepLoopStart") !=
         string::npos) ||
        ((CI->getCalledFunction()->getName()).find("qasmRepLoopStart") !=
         string::npos)) {
  ...
  if ((*mpItr).second.size() > 0) {
    ...
    for (unsigned mIndex = 0; mIndex < (*mfvIt).second.size(); mIndex++) {
      ...
      if (fToPrint.find("qasmRepLoopStart") != string::npos) {
        ...
        for (vector<qGateArg>::iterator
                 vpIt = (*mfvIt).second[mIndex].qArgs.begin(),
                 vpItE = (*mfvIt).second[mIndex].qArgs.end();
             vpIt != vpItE - 1; ++vpIt) {
          if ((*vpIt).isUndef)
            errs() << " UNDEF ";
          else {
            if ((*vpIt).isQbit || (*vpIt).isCbit) {
              ...
              if ((*vpIt).isPtr) // NOTE: not expecting non-quantum pointer
                                 ...
                                 // If they exist, then print out name of
                                 // variable
                errs() << " UNRECOGNIZED ";
