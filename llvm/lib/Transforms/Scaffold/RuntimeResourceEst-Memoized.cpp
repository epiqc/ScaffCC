...
struct RTResourceEst_Mem : public ModulePass {
  ...
  void visitCallInst(BasicBlock::iterator I) {
    ...
    const DataLayout &DL = CF->getParent()->getDataLayout();
    ...
    if (CF->isIntrinsic()) {
      ...
      Constant *StrConstant =
          ConstantDataArray::getString(CI->getContext(), CF->getName());
      ArrayType *strTy = cast<ArrayType>(StrConstant->getType());
      AllocaInst *strAlloc = new AllocaInst(strTy, AS, "", (Instruction *)CI);
      ...
      Constant *IntArrayConstant = ConstantDataArray::get(
          CI->getContext(), ArrayRef<uint32_t>(valueOfInt));
      ArrayType *intArrTy = cast<ArrayType>(IntArrayConstant->getType());
      AllocaInst *intArrAlloc = new AllocaInst(intArrTy, AS, "", (Instruction *)CI);
      ...
      Constant *DoubleArrayConstant = ConstantDataArray::get(
          CI->getContext(), ArrayRef<double>(valueOfDouble));
      ArrayType *doubleArrTy = cast<ArrayType>(DoubleArrayConstant->getType());
      AllocaInst *doubleArrAlloc =
          new AllocaInst(doubleArrTy, AS, "", (Instruction *)CI);
      ...
      Constant *DoubleNumConstant =
          ConstantInt::get(Type::getInt32Ty(CF->getContext()), num_doubles);
  ...
  void visitFunction(Function &F) {
    ...
    // insert exit_scope instruction in other quantum modules for end of scope
    // indication
  ...
  bool runOnModule(Module &M) {
    ...
    for (Module::iterator F = M.begin(); F != M.end(); ++F) {
      ...
      // the "exit_scope()" call for all functions, and they must always be at
      // the end
    }
    ...
    for (vector<BranchInst *>::iterator iterInst = vBranchReplace.begin();
      ...
      ICmpInst *test =
          new ICmpInst(*currentBlock, CmpInst::ICMP_EQ, memoizeInstruction,
...
static RegisterPass<RTResourceEst_Mem>
    X("runtime-resource-estimation-memoized",
      "Estimate qbits and qgates at runtime with memoization");
