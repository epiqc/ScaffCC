...
struct qGateArg { // arguments to qgate calls
  ...
  // the program.
  qGateArg()
      : argPtr(NULL), argNum(-1), isQbit(false), isAbit(false), isCbit(false),
        isParam(false), isUndef(false), isPtr(false), isDouble(false),
        numDim(0), valOrIndex(-1), val(0.0) {}
};
...
struct Optimize : public ModulePass {
  ...
  void print_qgateArg(qGateArg qg) {
    ...
    errs() << "  Arg Num: " << qg.argNum << "\n"
           ...
           << "  isAbit: " << qg.isAbit << "  isCbit: " << qg.isCbit
           << "  isPtr: " << qg.isPtr << "\n"
...
bool Optimize::backtraceOperand(Value *opd, int opOrIndex) {
  ...
  if (opOrIndex == 0) // backtrace for operand
  {
    ...
    if (GetElementPtrInst *GEPI = dyn_cast<GetElementPtrInst>(opd)) {
      ...
      else if (GEPI->hasIndices()) { // NOTE: Edit this function for multiple
                                     // indices, some of which are constant,
                                     // others are not.
#if 0
        errs()
            << "Oh no! I don't know how to handle this case..ABORT ABORT..\n";
#endif
        Instruction *pInst = dyn_cast<Instruction>(opd);
        unsigned numOps = pInst->getNumOperands();
        if (debugOptimize)
...
void Optimize::analyzeAllocInst(Function *F, Instruction *pInst) {
  if (AllocaInst *AI = dyn_cast<AllocaInst>(pInst)) {
    ...
    if (ArrayType *arrayType = dyn_cast<ArrayType>(allocatedType)) {
      ...
      tmpQArg.numDim = 1;
      tmpQArg.dimSize[0] =
          cast<ConstantInt>(AI->getArraySize())->getSExtValue();
      ...
      */
      ...
      if (elementType->isIntegerTy(16)) {
        ...
        bool foundit = false;
        for (vector<qGateArg>::iterator vParamIter = funcArgList.begin();
             (vParamIter != funcArgList.end() && !foundit); ++vParamIter) {
...
void Optimize::printFuncHeader(Function *F, bool lastFunc) {
  ...
  if (tmp_num_elem > 0) {
    ...
    errs() << " "
           << printVarName((funcArgList[tmp_num_elem - 1]).argPtr->getName());
  }

  errs() << " ) {\n ";

  // print qbits declared in function
  for (vector<qGateArg>::iterator vvit = qbitsInitInFunc.begin(),
                                  vvitE = qbitsInitInFunc.end();
       vvit != vvitE; ++vvit) {
    if ((*vvit).isQbit)
...
void Optimize::genGateArray(Function *F) {
  ...
  for (unsigned mIndex = 0; mIndex < mapFunction.size(); mIndex++) {
    if (mapFunction[mIndex].qArgs.size() > 0) {
      ...
      if (fToPrint.find("CNOT") != string::npos)
        fToPrint = "CNOT";
      else if (fToPrint.find("Toffoli.") != string::npos)
        fToPrint = "Toffoli";
      else if (fToPrint.find("MeasX") != string::npos)
        fToPrint = "MeasX";
      else if (fToPrint.find("MeasZ") != string::npos)
        fToPrint = "MeasZ";
      else if (fToPrint.find("H.i") != string::npos)
        fToPrint = "H";
      else if (fToPrint.find("Fredkin") != string::npos)
        fToPrint = "Fredkin";
      else if (fToPrint.find("PrepX") != string::npos)
        fToPrint = "PrepX";
      else if (fToPrint.find("PrepZ") != string::npos)
        fToPrint = "PrepZ";
      else if (fToPrint.substr(0, 2) == "Rz")
        fToPrint = "Rz";
      else if (fToPrint.find("S.") != string::npos)
        fToPrint = "S";
      else if (fToPrint.find("T.") != string::npos)
        fToPrint = "T";
      else if (fToPrint.find("Sdag") != string::npos)
        fToPrint = "Sdag";
      else if (fToPrint.find("Tdag") != string::npos)
        fToPrint = "Tdag";
      else if (fToPrint.find("X.") != string::npos)
        fToPrint = "X";
      else if (fToPrint.find("Z.") != string::npos)
        fToPrint = "Z";
      ...
      for (vector<qGateArg>::iterator vpIt = mapFunction[mIndex].qArgs.begin(),
                                      vpItE = mapFunction[mIndex].qArgs.end();
           vpIt != vpItE - 1; ++vpIt) {
        if ((*vpIt).isUndef)
          errs() << " UNDEF ";
        else {
          if ((*vpIt).isQbit || (*vpIt).isCbit || (*vpIt).isAbit) {
            ...
            if ((*vpIt).isPtr) // NOTE: not expecting non-quantum pointer
                               // variables as arguments to quantum functions.
                               // If they exist, then print out name of variable
              errs() << " UNRECOGNIZED ";
...
bool Optimize::ifCommute(OpGate G1, OpGate G2) {
  ...
  if (G1.gateTy == 't' || G1.gateTy == 's' || G1.gateTy == 'S' ||
      G1.gateTy == 'T') {
    G1st = true;
  }
  if (G2.gateTy == 't' || G2.gateTy == 's' || G2.gateTy == 'S' ||
      G2.gateTy == 'T') {
    G2st = true;
  }

  if (G1.gateTy == '\0' || G2.gateTy == '\0') {
    return true;
  }

  if (G1.gateTy != 'c' && G1.gateTy != 'o' && G2.gateTy != 'c' &&
      G2.gateTy != 'o') {
    ...
    if (G1.target != G2.target && G1.target != G2.control1 &&
        G1.target != G2.control2) {
      return true;
    }
    if (G1st && G1.target != G2.target) {
      return true;
    }
    return false;
  }
    ...
    if (G2.target != G1.target && G2.target != G1.control1 &&
        G2.target != G1.control2) {
      return true;
    }
    if (G2st && G1.target != G2.target) {
      return true;
    }
    return false;
  }

  else if (G1.target != G2.control1 && G1.target != G2.control2 &&
           G2.target != G1.control1 && G2.target != G1.control2) {
    return true;
  }
  return false;
}
...
bool Optimize::shareBit(OpGate G1, OpGate G2) {
  if ((G1.target == G2.target && G1.target != 0) ||
      (G1.target == G2.control1 && G1.target != 0) ||
      (G1.target == G2.control2 && G1.target != 0) ||
      (G1.control1 == G2.control2 && G1.control1 != 0) ||
      (G1.control1 == G2.control1 && G1.control1 != 0) ||
      (G1.control1 == G2.target && G1.control1 != 0) ||
      (G1.control2 == G2.target && G1.control2 != 0) ||
      (G1.control2 == G2.control1 && G1.control2 != 0) ||
      (G1.control2 == G2.control2 && G1.control2 != 0)) {
    return true;
  } else {
    return false;
  }
...
// Convert llvm internal data structure to OpGate for cleaner qubit argument(of
// gate) presentation
// and easier qubit map later
void Optimize::optimal_initial(Function *F, vector<string> &B,
                               vector<OpGate> &G) {
  ...
  for (unsigned mIndex = 0; mIndex < mapFunction.size(); mIndex++) {
    ...
    if (fName.find("H.") != string::npos) {
      nxtGate.gateTy = 'h';
    } else if (fName.find("CNOT") != string::npos) {
      nxtGate.gateTy = 'c';
    } else if (fName.find("Tof") != string::npos) {
      nxtGate.gateTy = 'o';
    } else if (fName.find("MeasX") != string::npos) {
      nxtGate.gateTy = 'X';
    } else if (fName.find("MeasZ") != string::npos) {
      nxtGate.gateTy = 'Z';
    } else if (fName.find("PrepX") != string::npos) {
      nxtGate.gateTy = 'n';
    } else if (fName.find("PrepZ") != string::npos) {
      nxtGate.gateTy = 'm';
    } else if (fName.find("Sdag") != string::npos) {
      nxtGate.gateTy = 'S';
    } else if (fName.find("S.") != string::npos) {
      nxtGate.gateTy = 's';
    } else if (fName.find("T.") != string::npos) {
      nxtGate.gateTy = 't';
    } else if (fName.find("Tdag") != string::npos) {
      nxtGate.gateTy = 'T';
    } else if (fName.substr(0, 2) == "Rz") {
      nxtGate.gateTy = 'r';
    } else if (fName.find("X.") != string::npos) {
      nxtGate.gateTy = 'x';
    } else if (fName.find("Z.") != string::npos) {
      nxtGate.gateTy = 'z';
    }
  ...
  temp_begin:
    ...
    if (G[gInd].gateTy == 'c') {
      ...
      while (((gOff + gInd) < G.size()) && gCount < OPT_THRESHOLD) {
        if (G[gInd + gOff].gateTy == 'c' &&
            G[gInd + gOff].target == G[gInd].target &&
            G[gInd + gOff].control1 == G[gInd].control1) {
        ...
        if (G[gInd + gOff].gateTy == 'c' &&
            G[gInd + gOff].target == G[gInd].control1 &&
            G[gInd + gOff].control1 == G[gInd].target) {
        ...
        gOff++;
        if (shareBit(G[gInd], G[gInd + gOff])) {
          gCount++;
        }
      }
    }
...
bool Optimize::runOnModule(Module &M) {
  ...
  bool hasMain = false;
  for (vector<Function *>::iterator it = qFuncs.begin(); it != qFuncs.end();
       it++) {
    if ((*it)->getName() == "main")
      hasMain = true;
  }
  ...
  for (vector<Function *>::iterator it = qFuncs.begin(); it != qFuncs.end();
       it++) {
    if ((*it)->getName() == "main") // assuming after flattening, the modules
                                    // left are Toffoli and main, only care
                                    // about main.
    {
      mapFunction = mapMapFunc.find(*it)->second;
      // Convert llvm internal data structure to OpGate for cleaner qubit
      // argument(of gate) presentation
      // and easier qubit map later
      ...
      for (unsigned i = 0; i < gateList.size(); i++) {
        ...
        if (gateList[i].gateTy == 'h') {
          errs() << "H ";
        }
        if (gateList[i].gateTy == 'c') {
          errs() << "CNOT ";
        }
        if (gateList[i].gateTy == 'o') {
          errs() << "Tof ";
        }
        if (gateList[i].gateTy == 'X') {
          errs() << "MeasX ";
        }
        if (gateList[i].gateTy == 'Z') {
          errs() << "MeasZ ";
        }
        if (gateList[i].gateTy == 'n') {
          errs() << "PrepX ";
        }
        if (gateList[i].gateTy == 'm') {
          errs() << "PrepZ ";
        }
        if (gateList[i].gateTy == 'S') {
          errs() << "Sdag ";
        }
        if (gateList[i].gateTy == 's') {
          errs() << "S ";
        }
        if (gateList[i].gateTy == 't') {
          errs() << "T ";
        }
        if (gateList[i].gateTy == 'T') {
          errs() << "Tdag ";
        }
        if (gateList[i].gateTy == 'r') {
          errs() << "Rz ";
        }
        if (gateList[i].gateTy == 'x') {
          errs() << "X ";
