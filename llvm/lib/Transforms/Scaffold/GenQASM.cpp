...
void GenQASM::genQASM(Function *F) {
  ...
  for (unsigned mIndex = 0; mIndex < mapFunction.size(); mIndex++) {
    if (mapFunction[mIndex].qArgs.size() > 0) {
      ...
      if (fToPrint.find("llvm.lifetime") != string::npos)
        continue; // Disable LifetimeMarkers
      ...
      if (fToPrint.find("Meas") != string::npos) {
        ...
        if (mvq != mapInstRtn.end()) {
          string VarName = printVarName(((*mvq).second).argPtr->getName());
          errs() << VarName;
          if (((*mvq).second).isPtr)
      ...
      for (vector<qGateArg>::iterator vpIt = mapFunction[mIndex].qArgs.begin(),
        ...
        if ((*vpIt).isUndef)
          ...
          if ((*vpIt).isQbit || (*vpIt).isCbit || (*vpIt).isAbit) {
            string VarName = printVarName((*vpIt).argPtr->getName());
            errs() << VarName;
            if (!((*vpIt).isPtr)) {
      ...
      if (tmpQA.isUndef)
        ...
        if (tmpQA.isQbit || tmpQA.isCbit || tmpQA.isAbit) {
          string VarName = "";
          if (tmpQA.argPtr) {
            VarName = printVarName(tmpQA.argPtr->getName());
            errs() << VarName;
          }
          if (!(tmpQA.isPtr)) {
          ...
          // variables as arguments to quantum functions. If they exist, then
          // print out name of variable
          if (tmpQA.isPtr)
            errs() << " UNRECOGNIZED ";
...
bool GenQASM::runOnModule(Module &M) {
  ...
  for (vector<Function *>::iterator it = qFuncs.begin(); it != qFuncs.end();
    ...
    if (newName.find("CNOT") != string::npos)
      newName = "CNOT";
    else if (newName.find("NOT.") != string::npos)
      newName = "X";
    else if (newName.find("Toffoli.") != string::npos)
      newName = "Toffoli";
    else if (newName.find("MeasX") != string::npos)
      newName = "MeasX";
    else if (newName.find("MeasZ") != string::npos)
      newName = "MeasZ";
    else if (newName.find("H.i") != string::npos)
      newName = "H";
    else if (newName.find("Fredkin") != string::npos)
      newName = "Fredkin";
    else if (newName.find("PrepX") != string::npos)
      newName = "PrepX";
    else if (newName.find("PrepZ") != string::npos)
      newName = "PrepZ";
    else if (newName.substr(0, 2) == "Rz")
      newName = "Rz";
    else if (newName.find("S.") != string::npos)
      newName = "S";
    else if (newName.find("T.") != string::npos)
      newName = "T";
    else if (newName.find("Sdag") != string::npos)
      newName = "Sdag";
    else if (newName.find("Tdag") != string::npos)
      newName = "Tdag";
    else if (newName.find("X.") != string::npos)
      newName = "X";
    else if (newName.find("Z.") != string::npos)
      newName = "Z";
    ...
    (*it)->setName(newName);
    if (it == lastItPos)
      printFuncHeader((*it), true);
    else
      printFuncHeader((*it), false);
    genQASM((*it));
  }
