...
struct qGateArg { // arguments to qgate calls
  ...
  double angle;
  qGateArg()
      : argPtr(NULL), argNum(-1), isQbit(false), isCbit(false), isAbit(false),
        isUndef(false), isPtr(false), valOrIndex(-1), angle(0.0) {}
};
...
static RegisterPass<GenSIMDSched> X("GenSIMDSchedule",
                                    "Generate SIMD Schedule");
  ...
  if (opOrIndex == 0) // backtrace for operand
  {
    ...
    if (GetElementPtrInst *GEPI = dyn_cast<GetElementPtrInst>(opd)) {
      ...
      if (GEPI->hasAllConstantIndices()) {
        ...
        if ((tmpDepQbit[0].isQbit || tmpDepQbit[0].isAbit) &&
            !(tmpDepQbit[0].isPtr)) {
...
uint64_t GenSIMDSched::get_ts_to_schedule(Function *F, uint64_t ts,
...
uint64_t GenSIMDSched::get_ts_to_schedule_leaf(Function *F, uint64_t ts,
                                               Function *funcToSched,
                                               uint64_t &first_step) {
...
void GenSIMDSched::cleanupCurrArrParGates() { currArrParGates.clear(); }
...
void GenSIMDSched::print_scheduled_gate(qGate qg, uint64_t ts) {
  ...
  if (tmpGateName.find("CNOT.") != string::npos)
    tmpGateName = "CNOT";
  else if (tmpGateName.find("NOT.") != string::npos)
    tmpGateName = "X";
  else if (tmpGateName.find("Toffoli.") != string::npos)
    tmpGateName = "Toffoli";
  else if (tmpGateName.find("MeasX.") != string::npos)
    tmpGateName = "MeasX";
  else if (tmpGateName.find("MeasZ.") != string::npos)
    tmpGateName = "MeasZ";
  else if (tmpGateName.find("H.i") != string::npos)
    tmpGateName = "H";
  else if (tmpGateName.find("Fredkin.") != string::npos)
    tmpGateName = "Fredkin";
  else if (tmpGateName.find("PrepX.") != string::npos)
    tmpGateName = "PrepX";
  else if (tmpGateName.find("PrepZ.") != string::npos)
    tmpGateName = "PrepZ";
  else if (tmpGateName.substr(0, 2) == "Rz")
    tmpGateName = "Rz";
  else if (tmpGateName.find("S.") != string::npos)
    tmpGateName = "S";
  else if (tmpGateName.find("T.") != string::npos)
    tmpGateName = "T";
  else if (tmpGateName.find("Sdag.") != string::npos)
    tmpGateName = "Sdag";
  else if (tmpGateName.find("Tdag.") != string::npos)
    tmpGateName = "Tdag";
  else if (tmpGateName.find("X.") != string::npos)
    tmpGateName = "X";
  else if (tmpGateName.find("Z.") != string::npos)
    tmpGateName = "Z";
