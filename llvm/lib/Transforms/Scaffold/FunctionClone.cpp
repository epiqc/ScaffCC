...
static RegisterPass<FunctionClone> X("FunctionClone", "Function Cloning Pass",
                                     false, false);

Function *FunctionClone::CloneFunctionInfo(
...
void FunctionClone::insertNewCallSite(CallInst *CI, std::string specializedName,
                                      Module *M) {
  ...
  for (unsigned i = 0,
                e = CI->getCalledFunction()->getFunctionType()->getNumParams();
       i != e; ++i, ++AI) // copy arguments FIXME: delete int args
    Args.push_back(*AI);

  ArrayRef<Value *> ArgsRef(Args);

  CallInst *newCall = CallInst::Create(M->getFunction(specializedName), ArgsRef,
                                       "", (Instruction *)CI);
...
bool FunctionClone::runOnModule(Module &M) {
  ...
  for (std::vector<Function *>::iterator vit = vectPostOrder.begin(),
    ...
    // what instructions (call sites) need to be erased after this function has
    // been processed
    std::vector<Instruction *> instErase;

    // in vectPostOrder traversal, when reaching a function that has been marked
    // for deletion...
    // skip and do not inspect it anymore
    if (std::find(funcErase.begin(), funcErase.end(), (*vit)) !=
        funcErase.end()) {
      if (debugCloning)
    ...
    for (inst_iterator I = inst_begin(*f), E = inst_end(*f); I != E; ++I) {
      ...
      if (CallInst *CI = dyn_cast<CallInst>(pInst)) {
        ...
        if (!CI->getCalledFunction()->isIntrinsic() &&
            !CI->getCalledFunction()->isDeclaration() && isQuantumModuleCall) {
          // first, find the argument positions of all integers and doubles
          // (regardless of being contstant or not)
          std::vector<unsigned> posOfInt;
          std::vector<unsigned> posOfDouble;
          for (unsigned iop = 0; iop < CI->getNumArgOperands(); iop++) {
            if (dyn_cast<ConstantInt>(CI->getArgOperand(iop)) ||
                CI->getArgOperand(iop)->getType() ==
                    Type::getInt32Ty(CI->getContext()))
              posOfInt.push_back(iop);
            if (dyn_cast<ConstantFP>(CI->getArgOperand(iop)) ||
                CI->getArgOperand(iop)->getType() ==
                    Type::getDoubleTy(CI->getContext()))
              posOfDouble.push_back(iop);
          }

          // second, scan for constant int or double arguments and save the
          // argument position with the constant value
          std::map<unsigned, int> valueOfInt;
          std::map<unsigned, double> valueOfDouble;
          for (unsigned iop = 0; iop < CI->getNumArgOperands(); iop++) {
            if (ConstantInt *CInt =
                    dyn_cast<ConstantInt>(CI->getArgOperand(iop)))
              valueOfInt[iop] = CInt->getZExtValue();
            if (ConstantFP *CDouble =
                    dyn_cast<ConstantFP>(CI->getArgOperand(iop)))
              // round to 4 decimanl points
              valueOfDouble[iop] =
                  floorf(CDouble->getValueAPF().convertToDouble() * 10000 +
                         0.5) /
                  10000;
          }
          ...
          if (!valueOfInt.empty() || !valueOfDouble.empty()) {
            ...
            // check to see if this function has itself been cloned before as a
            // result of this same pass
            // if not, pad with "_IPx", _MAX_INT_PARAMS times / "_DPx",
            // _MAX_DOUBLE_PARAMS times.
            // (it's like a fence to ensure after this point all to-be-cloned
            // modules got IPx_.._IPx_DPx_.._DPX placeholder in their names)
            ...
            // read the current value of integers and doubles from the
            // originalName string
            std::string::size_type found_pos_begin, found_pos_end,
                found_pos_begin_new;
            ...
            while (found_pos_begin != std::string::npos) {
              // because there might be numbers more than 1 digit long, need to
              // find begin and end
              found_pos_end = originalName.find_first_not_of(
                  "012345679-x", found_pos_begin + 3);
              std::string intString = originalName.substr(
                  found_pos_begin + 3, found_pos_end - (found_pos_begin + 3));
            ...
            while (found_pos_begin != std::string::npos) {
              ...
              // find begin and end
              found_pos_end = originalName.find_first_not_of(
                  "0123456789e.-x", found_pos_begin + 3);
              std::string doubleString = originalName.substr(
                  found_pos_begin + 3, found_pos_end - (found_pos_begin + 3));
            ...
            // Construct the specializedName string. All the above work was done
            // because this has to preserve the same order of arguments.
            // Check in posOfInt to see where this constant argument lies (0, 1,
            // 2, .., _MAX_NUM_INTS-1). Add current argument in that place.
            std::stringstream ss;

            // fill in the originalInts and originalDoubles with the new
            // constant values of this call site, at the correct position
            for (std::map<unsigned, int>::iterator i = valueOfInt.begin(),
                                                   e = valueOfInt.end();
                 i != e; ++i) {
              unsigned pos =
                  std::find(posOfInt.begin(), posOfInt.end(), i->first) -
                  posOfInt.begin();
            ...
            for (std::map<unsigned, double>::iterator i = valueOfDouble.begin(),
                                                      e = valueOfDouble.end();
                 i != e; ++i) {
              unsigned pos =
                  std::find(posOfDouble.begin(), posOfDouble.end(), i->first) -
                  posOfDouble.begin();
            ...
            for (std::vector<std::string>::iterator i = originalInts.begin(),
                                                    e = originalInts.end();
                 i != e; ++i)
              ss << "_IP" << *i;
            for (std::vector<std::string>::iterator j = originalDoubles.begin(),
                                                    e = originalDoubles.end();
                 j != e; ++j) {
            ...
            if (debugCloning)
              errs() << "\t\tCloned Function: "
                     << specializedFunction->getName() << "\n";

            // Iterate over function arguments to apply constants to VMap
            for (Function::arg_iterator i = F->arg_begin(), ie = F->arg_end();
                 i != ie; ++i) {
              Argument *arg = i;
              ...
              if (valueOfInt.count(argNo) == 1) {
                ...
                Value *val;
                val = ConstantInt::get(Type::getInt32Ty(M.getContext()),
                                       valueOfInt[argNo]);
                ...
                Value *val;
                val = ConstantFP::get(Type::getDoubleTy(M.getContext()),
                                      valueOfDouble[argNo]);
                WeakTrackingVH wval(val);
                VMap[i] = wval;
              }
            ...
            // once a Function is cloned, it is a candidate for removal from
            // vector
            // mark for deletion
            if (std::find(funcErase.begin(), funcErase.end(), F) ==
                funcErase.end())
              funcErase.push_back(F);

            // insert this new cloned function into the vector
            std::vector<Function *>::iterator it =
                std::find(vectPostOrder.begin(), vectPostOrder.end(), F);
            vectPostOrder.insert(it, specializedFunction);

            // the insertion will invalidate vit - reassign the vector to be
            // safe
    ...
    // remove instructions (call sites) that called the original (before
    // cloning) function
    for (std::vector<Instruction *>::iterator i = instErase.begin(),
                                              e = instErase.end();
         i != e; ++i)
      (*i)->eraseFromParent();
