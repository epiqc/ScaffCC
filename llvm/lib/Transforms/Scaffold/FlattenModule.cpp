...
static cl::opt<unsigned> FLAT("all", cl::init(0), cl::Hidden,
...
static RegisterPass<FlattenModule>
    X("FlattenModule", "Quantum Module Flattening Pass", false, false);
...
bool FlattenModule::runOnModule(Module &M) {
  ...
  for (std::vector<std::string>::iterator i = leafNames.begin(),
                                          e = leafNames.end();
       i != e; ++i) {
    if (debugFlattening)
      errs() << "flat_info: " << *i << "\n";
...
bool FlattenModule::runOnFunction(Function &F) {
  ...
  // only continue if this function is part of makeLeaf and complete inlining is
  // not toggled on
  if (FLAT == 0) {
