...
struct GetCriticalPath : public ModulePass {
  ...
  vector<qArgInfo>
      currTimeStep; // contains set of arguments operated on currently
  vector<string> currParallelFunc;
  MaxInfo maxParallelFactor; // overall max parallel factor
  map<string, int> gate_index;
  vector<uint64_t> curr_parallel_ts; // vector of current timesteps that are
                                     // parallel; used for comparing functions
  ...
  map<string, map<int, uint64_t> >
      funcQbitsStart; // start ts of qbits in current function
...
void GetCriticalPath::init_critical_path_algo(Function *F) {
  ...
  // clear tsGates
  for (map<uint64_t, vector<qGate> >::iterator mit = tsGates.begin();
       mit != tsGates.end(); ++mit) {
    (*mit).second.clear();
  }
  tsGates.clear();
...
void GetCriticalPath::print_funcQbitsHalf() {
  ...
  for (map<string, map<int, uint64_t> >::iterator mIter = funcQbitsHalf.begin();
       mIter != funcQbitsHalf.end(); ++mIter) {
    errs() << "Var " << (*mIter).first << " ---> ";
    for (map<int, uint64_t>::iterator indexIter = (*mIter).second.begin();
         indexIter != (*mIter).second.end(); ++indexIter) {
      errs() << (*indexIter).first << ":" << (*indexIter).second << "  ";
...
void GetCriticalPath::update_critical_info(string currFunc, uint64_t ts,
                                           string fname) {
  map<string, allTSParallelism>::iterator fitr = funcParallelFactor.find(fname);
  map<string, allTSParallelism>::iterator citr =
      funcParallelFactor.find(currFunc);
  assert(fitr != funcParallelFactor.end() &&
         "parallel func not found in funcParallelFactor");

  assert(citr != funcParallelFactor.end() &&
         "curr func not found in funcParallelFactor");
  ...
  if (ts + newTSsize <= currTSsize) {
    for (unsigned i = 0; i < newTSsize; i++) {
      for (int k = 0; k < NUM_QGATES; k++) {
        (*citr).second.gates[ts + i].parallel_gates[k] +=
            (*fitr).second.gates[i].parallel_gates[k];
      }
    }
  } else {
...
void GetCriticalPath::print_tableFuncQbitsStart() {
  ...
  for (map<Function *, map<unsigned int, map<int, uint64_t> > >::iterator m1 =
           tableFuncQbitsStart.begin();
       m1 != tableFuncQbitsStart.end(); ++m1) {
    errs() << "Function " << (*m1).first->getName() << " \n  ";
    for (map<unsigned int, map<int, uint64_t> >::iterator m2 =
             (*m1).second.begin();
         m2 != (*m1).second.end(); ++m2) {
...
void GetCriticalPath::calc_max_parallelism_statistic() {
  map<string, allTSParallelism>::iterator fitr =
      funcParallelFactor.find("main");
  assert(fitr != funcParallelFactor.end() && "Func not found in funcParFac");
  errs() << "Timesteps = " << (*fitr).second.timesteps << "\n";
  ...
  errs() << "Max parallelism in types of gates = " << max_par
         << " in TS: " << ts_par << "\n";
}
...
void GetCriticalPath::print_tsGates() {
  for (map<uint64_t, vector<qGate> >::iterator mit = tsGates.begin();
       mit != tsGates.end(); ++mit) {
    errs() << "TS#" << (*mit).first << " --> ";
    for (vector<qGate>::iterator vit = (*mit).second.begin();
...
uint64_t GetCriticalPath::compute_least_slack(Function *F, qGate qg,
                                              uint64_t tmax) {
  ...
  map<Function *, map<unsigned int, map<int, uint64_t> > >::iterator tableIt =
      tableFuncQbitsStart.find(qg.qFunc);
  assert(tableIt != tableFuncQbitsStart.end() &&
         "No previous entry for this function");

  for (int i = 0; i < qg.numArgs; i++) {
    map<unsigned int, map<int, uint64_t> >::iterator entryIt =
        (*tableIt).second.find(i);
    if (entryIt != (*tableIt).second.end()) {
      ...
      if (qg.args[i].index == -1) { // qbit*
        ...
        assert(lookUpQbitIt != (*entryIt).second.end() &&
               "arg index not found in tablefuncqbitshalf"); // there exists
                                                             // entry for reqd
                                                             // index in the
                                                             // func table of
                                                             // called func
...
void GetCriticalPath::calc_critical_time(Function *F, qGate qg) {
  ...
  if (isFirstMeas && (fname.find("llvm.MeasX") != std::string::npos ||
      fname.find("llvm.MeasZ") != std::string::npos)) {
    ...
    // update_critical_info(F->getName().str(), maxFQ, qg.qFunc->getName(),
    // qg.angle);
    isFirstMeas = false;
    ...
    if (fname.find("llvm.") != string::npos) { // is intrinsic
      ...
      for (int i = 0; i < qg.numArgs; i++) {
        ...
        if (argIndex == -1) {
          ...
          // update the timestep number for that argument
          map<int, uint64_t>::iterator indexIter =
              (*mIter).second.find(argIndex);
      ...
      // small code to ensure that any ancilla processing that does not fall
      // within this function's critical path get a cycle in the schedule.
      ...
      uint64_t tmpDelay =
          max_ts_of_all_args + (*delayIt).second - least_slack + 1;
      if (tmpDelay > highestDelay)
        highestDelay = tmpDelay;

      // check tableFuncQbits for values to update with
      map<Function *, map<unsigned int, map<int, uint64_t> > >::iterator tableIt =
          tableFuncQbits.find(qg.qFunc);
      assert(tableIt != tableFuncQbits.end() &&
             "No previous entry for this function");

      for (int i = 0; i < qg.numArgs; i++) {
        map<string, map<int, uint64_t> >::iterator mIter =
            funcQbits.find(qg.args[i].name);
        assert(mIter != funcQbits.end()); // should already have an entry for
                                          // the name of the qbit
        ...
        if (entryIt != (*tableIt).second.end()) {
          ...
          if (qg.args[i].index == -1) { // qbit*
            for (map<int, uint64_t>::iterator indexIt =
                     (*entryIt).second.begin();
                 indexIt != (*entryIt).second.end(); ++indexIt) {
              map<int, uint64_t>::iterator currEntryIt =
                  (*mIter).second.find((*indexIt).first);
              if (currEntryIt != (*mIter).second.end())
                (*currEntryIt).second =
                    max_ts_of_all_args + (*indexIt).second - least_slack + 1;
              else
                (*mIter).second[(*indexIt).first] =
                    max_ts_of_all_args + (*indexIt).second - least_slack + 1;
            }
          } else { // qbit was passed
            // take the 0th entry and add that to the index entry
            map<int, uint64_t>::iterator lookUpQbitIt =
                (*entryIt).second.find(0);
            assert(lookUpQbitIt != (*entryIt).second.end()); // there exists
                                                             // entry for 0 in
                                                             // the func table
                                                             // of called func
            map<int, uint64_t>::iterator currEntryIt =
                (*mIter).second.find(qg.args[i].index);
            if (currEntryIt != (*mIter).second.end())
              (*currEntryIt).second =
                  max_ts_of_all_args + (*lookUpQbitIt).second - least_slack + 1;
            else
              (*mIter).second[qg.args[i].index] =
                  max_ts_of_all_args + (*lookUpQbitIt).second - least_slack + 1;

            // update -2 entry for the array, i.e. max ts over all indices
            currEntryIt = (*mIter).second.find(-2);
            if ((*currEntryIt).second <
                max_ts_of_all_args + (*lookUpQbitIt).second)
              (*currEntryIt).second =
                  max_ts_of_all_args + (*lookUpQbitIt).second - least_slack + 1;
          }
        }
...
void GetCriticalPath::saveTableFuncQbitsStart(Function *F) {
  ...
  for (map<string, map<int, uint64_t> >::iterator mapIt = funcQbitsHalf.begin();
       mapIt != funcQbitsHalf.end(); ++mapIt) {
    map<string, unsigned int>::iterator argIt = funcArgs.find((*mapIt).first);
    if (argIt != funcArgs.end()) {
      unsigned int argNum = (*argIt).second;
      tmpFuncQbitsMap[argNum] = (*mapIt).second;
...
void GetCriticalPath::init_funcQbitsHalf(uint64_t i) {
  for (map<string, map<int, uint64_t> >::iterator inItr = funcQbits.begin();
       inItr != funcQbits.end(); ++inItr) {
    map<string, map<int, uint64_t> >::iterator outItr =
        funcQbitsHalf.find((*inItr).first);
    assert(outItr != funcQbitsHalf.end() &&
           "funcQbitsHalf does not have entry");

    for (map<int, uint64_t>::iterator in2Itr = (*inItr).second.begin();
         in2Itr != (*inItr).second.end(); ++in2Itr) {
  ...
  for (uint64_t i = hct + 1; i < ct; i++) {
    ...
    for (vector<qGate>::iterator vit = (*mit).second.begin();
      ...
      for (int j = 0; j < (*vit).numArgs; j++) {
        ...
        map<string, map<int, uint64_t> >::iterator fit =
            funcQbitsHalf.find(argName);
        ...
        assert(mfit != (*fit).second.end() &&
               "arg index not found in funcQbitsHalf");
...
void GetCriticalPath::schedule_alap_insts(uint64_t ct, uint64_t hct) {
  ...
  for (uint64_t i = hct; i >= 1; i--) {
    ...
    for (vector<qGate>::iterator vit = (*mit).second.begin();
      ...
      for (int j = 0; j < (*vit).numArgs; j++) {
        map<string, map<int, uint64_t> >::iterator mIter =
            funcQbitsHalf.find((*vit).args[j].name);
  ...
  for (scc_iterator<CallGraph *> sccIb = scc_begin(&CG);
    ...
    for (std::vector<CallGraphNode *>::const_iterator nsccI = nextSCC.begin(),
      ...
      if (F && !F->isDeclaration()) {
        ...
        errs() << F->getName() << " " << max(find_max_funcQbits(), highestDelay)
               << " isLeaf= " << isLeaf << "\n";
      } else {
        if (debugGetCriticalPath)
