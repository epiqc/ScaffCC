...
struct DynCritPath : public ModulePass, public InstVisitor<DynCritPath> {
  ...
  void initializeQbits(Instruction *CI) {
    ...
    for (map<AllocaInst *, int>::iterator mit = mapAllocaInst.begin();
         mit != mapAllocaInst.end(); ++mit) {
      ...
      idxVect.push_back(
          ConstantInt::get(Type::getInt32Ty(CI->getContext()), 0));
      GetElementPtrInst *arrPtr = GetElementPtrInst::Create(
          Type::getInt32Ty(CI->getContext()), (Value *)(*mit).first, idxVect,
          "", (Instruction *)CI);
      ...
      Value *intArg =
          ConstantInt::get(Type::getInt32Ty(CI->getContext()), arrSize);
      call_args.push_back(intArg);
  ...
  void resetAncillaData(Function &F) {
    ...
    for (map<AllocaInst *, int>::iterator mit = mapAllocaInst.begin();
      ...
      SmallVector<Value *, 16> idxVect;
      idxVect.push_back(
          ConstantInt::get(Type::getInt32Ty(BBTerm->getContext()), 0));
      idxVect.push_back(
          ConstantInt::get(Type::getInt32Ty(BBTerm->getContext()), 0));
      GetElementPtrInst *arrPtr = GetElementPtrInst::Create(
          Type::getInt32Ty(BBTerm->getContext()), (Value *)(*mit).first,
          idxVect, "", (Instruction *)BBTerm);

      // generate call inst to external store function
      SmallVector<Value *, 16> call_args;
      call_args.push_back(arrPtr);
      Value *intArg =
          ConstantInt::get(Type::getInt32Ty(BBTerm->getContext()), arrSize);
      call_args.push_back(intArg);
  ...
  bool runOnModule(Module &M) {
    ...
    dcpGate = cast<Function>(
        M.getOrInsertFunction("dcp_qgate", Type::getVoidTy(M.getContext()),
                              Type::getInt32Ty(M.getContext()),
                              Type::getInt16Ty(M.getContext()), (Type *)0));

    dcpGate2 = cast<Function>(M.getOrInsertFunction(
        "dcp_qgate2", Type::getVoidTy(M.getContext()),
        Type::getInt32Ty(M.getContext()), Type::getInt16Ty(M.getContext()),
        Type::getInt16Ty(M.getContext()), (Type *)0));

    dcpGate3 = cast<Function>(M.getOrInsertFunction(
        "dcp_qgate3", Type::getVoidTy(M.getContext()),
        Type::getInt32Ty(M.getContext()), Type::getInt16Ty(M.getContext()),
        Type::getInt16Ty(M.getContext()), Type::getInt16Ty(M.getContext()),
        (Type *)0));

    dcpQbitInit = cast<Function>(
        M.getOrInsertFunction("dcp_qbit_init", Type::getVoidTy(M.getContext()),
                              Type::getInt16Ty(M.getContext())->getPointerTo(),
                              Type::getInt32Ty(M.getContext()), (Type *)0));

    dcpAncReset = cast<Function>(
        M.getOrInsertFunction("dcp_anc_reset", Type::getVoidTy(M.getContext()),
                              Type::getInt16Ty(M.getContext())->getPointerTo(),
                              Type::getInt32Ty(M.getContext()), (Type *)0));

    dcpSumm = cast<Function>(M.getOrInsertFunction(
        "dcp_summary", Type::getVoidTy(M.getContext()), (Type *)0));

    dcpInitAlgo = cast<Function>(M.getOrInsertFunction(
        "dcp_init_algo", Type::getVoidTy(M.getContext()), (Type *)0));

    for (Function &F : M) {
      if (!F.isDeclaration()) {
        // analyze Alloc Insts first
        for (inst_iterator instIb = inst_begin(F); instIb != inst_end(F);
             ++instIb) {
...
static RegisterPass<DynCritPath>
    X("dyn-critical-path",
      "Instrument binary for dynamic estimation of critical path");
