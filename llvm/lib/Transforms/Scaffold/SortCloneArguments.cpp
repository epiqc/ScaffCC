...
static RegisterPass<SortCloneArguments>
    X("SortCloneArguments", "Sorting Arguments of Cloned Function", false,
      false);
...
bool SortCloneArguments::runOnModule(Module &M) {
  ...
  for (Module::iterator F = M.begin(); F != M.end(); ++F) {
    if (F->isIntrinsic() || F->isDeclaration() || F->getName() == "main")
      continue;
    //----- Only Modifying Function Names: Therefore the correspondence bewteen
    //name order and
    ...
    while (found_pos_begin != std::string::npos) {
      // because there might be numbers more than 1 digit long, need to find
      // begin and end
      found_pos_end =
          originalName.find_first_not_of("0123456789", found_pos_begin + 3);
    ...
    while (found_pos_begin != std::string::npos) {
      ...
      // begin and end
      found_pos_end =
          originalName.find_first_not_of("0123456789e.-", found_pos_begin + 3);
    ...
    std::stringstream ss;
    for (std::vector<int>::iterator i = originalInts.begin(),
                                    e = originalInts.end();
         i != e; ++i)
      ss << "_IP" << *i;
