  ...
  struct InsertReverseFunctionsVisitor
    ...
    InsertReverseFunctionsVisitor(Module *module) : M(module) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::H, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::H, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::MeasX, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::MeasX, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::MeasZ, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::MeasZ, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::PrepX, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::PrepX, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::PrepZ, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::PrepZ, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::Rx, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::Rx, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::Ry, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::Ry, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::Rz, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::Rz, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::S, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::Sdag, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::Sdag, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::S, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::T, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::Tdag, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::Tdag, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::T, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::X, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::X, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::Y, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::Y, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::Z, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::Z, ar);
      }

      for (int i = 0; i < 4; i++) {
        llvm::ArrayRef<Type *> ar = llvm::makeArrayRef(twoPerms[i]);
        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::CNOT, ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::CNOT, ar);
      }

      for (int i = 0; i < 8; i++) {
        llvm::ArrayRef<Type *> ar = llvm::makeArrayRef(threePerms[i]);
        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::Toffoli,
                                                    ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::Toffoli, ar);

        IntrinsicInverses[Intrinsic::getDeclaration(M, Intrinsic::Fredkin,
                                                    ar)] =
            Intrinsic::getDeclaration(M, Intrinsic::Fredkin, ar);
      }
    ...
    Function *GetOrCreateInverseFunction(Function *Func) {
      ...
      if (!ReverseFunc) {
        ...
        for (I = V.rbegin(); I != F; I++) {
          Inverse = GetOrCreateInverseFunction((*I)->getCalledFunction());
          (*I)->setCalledFunction(Inverse);
          (*I)->removeFromParent();
          BB.getInstList().insert(BB.back().getIterator(), *I);
        }
    ...
    void visitCallInst(CallInst &I) {
      ...
      if (CF->getName().startswith(REVERSE_PREFIX)) {
        ...
        while (ForwardFunctionName.startswith(REVERSE_PREFIX)) {
          ForwardFunctionName =
              ForwardFunctionName.drop_front(REVERSE_PREFIX.size());
          ++numReverses;
        }
        ...
        if (ForwardFunction == NULL) {
          ...
          ForwardFunction = M->getFunction(IntrinsicForwardFunctionName);
        }

        // If no function exists for the base after the
        // _reverse_, print an error message
        if (ForwardFunction == NULL) {
          errs() << "Error: Could not invert non-existent function "
                 << ForwardFunctionName << "\n";
        }
        ...
        else if (CF->getFunctionType() != ForwardFunction->getFunctionType()) {
...
static RegisterPass<FunctionReverse> X("FunctionReverse", "Function Reverser",
                                       false, false);
