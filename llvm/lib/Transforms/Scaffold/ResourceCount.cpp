...
struct ResourceCount : public ModulePass {
  ...
  void CountFunctionResources(
    ...
    for (inst_iterator I = inst_begin(*F), E = inst_end(*F); I != E; ++I) {
      ...
      // Gates?
                   ...
                   dyn_cast<CallInst>(Inst)) { // Filter Call Instructions
        ...
        if (callee->isIntrinsic()) { // Intrinsic (Gate) Functions calls
          ...
          errs() << "net then width" << FunctionResources[F][2]
          ...
          if (FunctionResources.find(callee) != FunctionResources.end()) {
            ...
            if (callee_numbers[3] >
                FunctionResources[F][3] - FunctionResources[F][2])
              FunctionResources[F][3] =
                  FunctionResources[F][2] + callee_numbers[3];
  ...
  virtual bool runOnModule(Module &M) {
    // Function* ---> Qubits | Gross Abits | Net Abits | Max Abit Width | X | Z
    // | H | T | CNOT | Toffoli | PrepZ | MeasZ | Rz | Ry
    ...
    // print results
    errs() << "\tQubit\tGross A\tNet "
              "A\tWidth\tX\tZ\tH\tT\tT_dag\tS\tS_dag\tCNOT\tPrepZ\tMeasZ\n";
    for (std::map<Function *, unsigned long long *>::iterator
      ...
      if (i->first->getName() == "main")
        app_total_gates = function_total_gates;
      errs() << "\n";
