...
static cl::opt<unsigned> METRICS("metrics", cl::init(0), cl::Hidden,
                                 cl::desc("Print Metrics"));

static cl::opt<unsigned> FULL_SCHED("full_sched", cl::init(0), cl::Hidden,
...
struct modularInfo {
  ...
  modularInfo()
      : width(0), length(0), tgates(0), tgates_ub(0), tgates_par(0),
        tgates_par_ub(0) {}
};
...
struct qArgInfo {
  ...
  qArgInfo()
      : name("none"), index(-1), id(-1), simd(0), loc(0), nextTS(-1),
        last_inst(NULL) {}
...
struct op {
  ...
  op()
      : name(), id(-1), ts(-1), dist(-1), followed(0), simd(-1), tag(0),
        path(0), label(NULL), in_edges(), out_edges() {}
};
...
struct qubit {
  ...
  int id;
  qubit()
      : name("none"), index(0), size(1), last_op(-1), ops(), simd(-1), id(-1) {}
};
...
struct GenLPFSSched : public ModulePass {
  ...
  vector<pair<Instruction *, op> >
      funcList; // list of the operations of a function
  ...
  map<Instruction *, op> mapCalls; // map of between the instruction label and
                                   // the operation attributes of each inst
  vector<Instruction *> longPath; // longest path to be returned by find_lp
  vector<Instruction *> callList;
  map<int, multimap<int, op> >
      schedule; // all the instructions in a given simd region
  int ots;      // operating time steps
  int simds;
  int tgates_cnt; // tgates
  multimap<int, move>
      move_schedule; // all the instructions in a given simd region
  multimap<int, move>
      local_move_schedule; // all the instructions in a given simd region
  int mts;                 // move time steps
  map<int, vector<Instruction *> >
      longestPathList; // all the instructions in a given simd region
...
static RegisterPass<GenLPFSSched> X("GenLPFSSchedule",
                                    "Generate LPFS Schedule");

// LPFS: Longest Path First Scheduling

vector<Instruction *> longestPathList;

void GenLPFSSched::lpfs(Function *F, int ts, int simd_l, int refill_simd,
                        int opp_simd) {
  ...
  for (vector<Instruction *>::iterator fp1 = callList.begin();
       fp1 != callList.end(); ++fp1) { // All function instructs
    ...
    for (int i = 0; i < (*mp1).second.name.numArgs; ++i) { // All function args
      ...
      while (fp2 != callList.end()) { // All later instructs
        ...
        found_one = false;
        for (int j = 0; j < (*mp2).second.name.numArgs;
             ++j) { // All those inst args
          if ((*mp1).second.name.args[i].id == -1) {
            (*mp1).second.name.args[i].id = qbit_id++;
            qArgInfo arg = (*mp1).second.name.args[i];
            stringstream ss;
  ...
  for (map<int, vector<Instruction *> >::iterator pathNumber =
           longestPathList.begin();
       pathNumber != longestPathList.end(); pathNumber++) {
    for (vector<Instruction *>::reverse_iterator inst =
             (*pathNumber).second.rbegin();
         inst != (*pathNumber).second.rend(); inst++) {
        ...
        if (depsMet(myInst, ts)) {
          if (opp_simd == 1) {
            while (simdToSched <= (int)RES_CONSTRAINT) {
              map<int, multimap<int, op> >::iterator it =
                  schedule.find(simdToSched);
              if (!(it == schedule.end())) {
            ...
            if (!scheduled) {
              ...
              int lowSD = 0;
              for (simdToSched = (int)RES_CONSTRAINT; simdToSched > 0;
                   simdToSched--) {
                int tempTS = ts;
            ...
            while (!schedule[simdToSched].count(ts))
              ts++;
            sched_op(myInst, ts, simdToSched);
            scheduled = true;
            sched_ops++;
...
void GenLPFSSched::update_moves(int moves, int ts) {
  ...
  for (vector<qArgInfo>::iterator mapit = active_qubits.begin();
       mapit != active_qubits.end(); mapit++) {
    stringstream ss;
    ss << (*mapit).index;
    ...
    while (qit != current.end()) {
      ...
      if ((name == name2) &&
          (thisQbit.loc % 10 !=
           0)) { // If qubit is in current and active and not in memory
        dest = (*qit).simd;
    ...
    if (!(simd_active[src]) && !(dest)) { // Qbit doesn't need to move
      ...
      if (!(dest) && (simd_active[src])) {
        ...
        if ((lowNextTS <= ts + (int)LOCAL_WINDOW) && (myOp.ts != ts) &&
            (nextOpLoc == myOp.simd) &&
            ((*qubitMap.find(name)).second.loc % 10 != 0)) {
          if ((src) && (localMemSizeMap[src * 10] >= (int)LOCAL_Q)) {
            int maxTS = 0;
            string maxName;
            qArgInfo victim;
            for (map<string, qArgInfo>::iterator mit = qubitMap.begin();
                 mit != qubitMap.end(); mit++) {
  ...
  for (vector<qArgInfo>::iterator mapit = current.begin();
       mapit != current.end(); mapit++) {
    stringstream ss;
    ss << (*mapit).index;
    string name = (*mapit).name + ss.str();

    qArgInfo curQbit = (*qubitMap.find(name)).second;
    int dest = curQbit.simd; // Region where qubit is needed for operation at
                             // current timestep
...
bool GenLPFSSched::depsMet(Instruction *currentOp, int currentTime) {
  ...
  if (!(myOp.in_edges.empty())) {
    for (vector<Instruction *>::iterator parent = myOp.in_edges.begin();
         parent != myOp.in_edges.end(); parent++) {
      if (((*mapCalls.find(*parent))).second.simd == -1)
        answer = false;
      if (((*mapCalls.find(*parent))).second.ts >= currentTime)
        answer = false;
    }
  }
  return answer;
...
void GenLPFSSched::sched_op(Instruction *currentOp, int timeStep, int simd) {
  if ((*mapCalls.find(currentOp)).second.simd == -1) {
    ...
    schedule[simd].insert(
        make_pair(timeStep, ((*mapCalls.find(currentOp)).second)));
    regionSizeMap[simd]++;
    if (((*mapCalls.find(currentOp)).second.name.qFunc->getName() ==
         "llvm.T") ||
        ((*mapCalls.find(currentOp)).second.name.qFunc->getName() ==
         "llvm.Tdag")) {
      tgates_cnt++;
    }
  }
...
void GenLPFSSched::find_lp(Function *F, int pathNum) {
  ...
  for (map<Instruction *, op>::iterator mp1 = mapCalls.begin();
       mp1 != (--mapCalls.end()); ++mp1) {
    if (!(*mp1).second.followed)
      (*mp1).second.dist = 1;
  }
  for (vector<Instruction *>::iterator fp1 = callList.begin();
       fp1 != (--callList.end()); ++fp1) {
    map<Instruction *, op>::iterator mp1 = mapCalls.find(*fp1);
    if ((*mp1).second.followed) {
      (*mp1).second.dist = 0;
    } else {
      ...
      if (!(thisOp.out_edges.empty())) {
        for (vector<Instruction *>::iterator child = thisOp.out_edges.begin();
             child != thisOp.out_edges.end(); child++) {
  ...
  if (!(longPath.empty())) {
    while ((*mapCalls.find(longPath[longPath.size() - 1])).second.dist > 1) {
      ...
      for (vector<Instruction *>::iterator parent = botOp.in_edges.begin();
           parent != botOp.in_edges.end(); ++parent) {
        if (((*mapCalls.find(*parent)).second.dist == currDist) &&
            (!(*mapCalls.find(*parent)).second.followed)) {
          longPath.push_back(*parent); // next operation is appended to the
                                       // path, so path vector is in reverse
          break;
        }
      }
    }
...
uint64_t GenLPFSSched::get_ts_to_schedule(Function *F, uint64_t ts,
...
uint64_t GenLPFSSched::get_ts_to_schedule_leaf(Function *F, uint64_t ts,
                                               Function *funcToSched,
                                               uint64_t &first_step) {
  ...
  map<string, int>::iterator gidx =
      gate_index.find(funcToSched->getName().str().substr(5));
...
void GenLPFSSched::cleanupCurrArrParGates() { currArrParGates.clear(); }
...
void GenLPFSSched::print_funcList() {
  if (!(funcList.empty())) {
    for (vector<pair<Instruction *, op> >::iterator f1 = funcList.begin();
         f1 != funcList.end(); ++f1) {
...
void GenLPFSSched::print_mapCalls() {
  for (map<Instruction *, op>::iterator mp = mapCalls.begin();
       mp != mapCalls.end(); ++mp) {
    errs() << "INSTRUCTION: " << (*mp).first << " timestep "
           << mapCalls[(*mp).first].ts
           << " Dist: " << mapCalls[(*mp).first].dist
           << " | Followed: " << mapCalls[(*mp).first].followed << " qGate: ";
...
void GenLPFSSched::print_priorityVector() {
  for (vector<InstPri>::iterator pvit = priorityVector.begin();
       pvit != priorityVector.end(); ++pvit)
    errs() << "#PRIORITY VECTOR ENTRY: "
           << (*mapCalls.find((*pvit).first)).second.name.qFunc->getName()
           << "\n";
}
...
void GenLPFSSched::print_longPath() {
  errs() << "\n Longest Path: \n";
  int i = 1;
  for (vector<Instruction *>::reverse_iterator rlp = longPath.rbegin();
       rlp != longPath.rend(); ++rlp) {
...
void GenLPFSSched::print_schedule(Function *F, int op_count) {
  ...
  while (ts < op_count) {
    ...
    while ((moveOper != move_schedule.end()) && ((*moveOper).first == ts)) {
      errs() << (*moveOper).first << ",0 TMOV " << (*moveOper).second.dest
             << " " << (*moveOper).second.src << " "
             << (*moveOper).second.arg.name << (*moveOper).second.arg.index
             << "\n";
      moveOper++;
    }
    while ((bmoveOper != local_move_schedule.end()) &&
           ((*bmoveOper).first == ts)) {
      errs() << (*bmoveOper).first << ",0 BMOV " << (*bmoveOper).second.dest
             << " " << (*bmoveOper).second.src << " "
             << (*bmoveOper).second.arg.name << (*bmoveOper).second.arg.index
             << "\n";
      bmoveOper++;
    }
    for (map<int, multimap<int, op> >::iterator pit = schedule.begin();
         pit != schedule.end(); pit++) {
      if (!(*pit).second.empty()) {
        ...
        while (oper != (*pit).second.end() && (*oper).first == ts) {
          ...
          if (tmpName.find("llvm.") != string::npos) {
            ...
            if (firstDotPos == secondDotPos)
              ...
              errs() << tmpName.substr(firstDotPos + 1,
                                       secondDotPos - firstDotPos - 1);
          } else
            errs() << tmpName;
          for (int i = 0; i < (*oper).second.name.numArgs; i++) {
            errs() << " " << (*oper).second.name.args[i].name;
            if ((*oper).second.name.args[i].index != -1)
              errs() << (*oper).second.name.args[i].index;
          }
          errs() << "\n";
          oper++;
        }
...
void GenLPFSSched::print_moves_schedule(Function *F, int op_count) {
  ...
  for (multimap<int, move>::iterator mit = move_schedule.begin();
       mit != move_schedule.end(); mit++) {
    while ((*mit).first == ts) {
      errs() << (*mit).first << ",0 TMOV " << (*mit).second.dest << " "
             << (*mit).second.src << " " << (*mit).second.arg.name
             << (*mit).second.arg.index << "\n";
      mit++;
    }
    ts++;
  }
...
void GenLPFSSched::print_local_moves_schedule(Function *F, int op_count) {
  ...
  for (mit = local_move_schedule.begin(); mit != local_move_schedule.end();
       mit++) {
    while ((*mit).first == ts) {
      errs() << (*mit).first << ",0 BMOV " << (*mit).second.dest << " "
             << (*mit).second.src << " " << (*mit).second.arg.name
             << (*mit).second.arg.index << "\n";
      mit++;
    }
...
void GenLPFSSched::print_schedule_metrics(Function *F, int op_count) {
  ...
  for (multimap<int, move>::iterator mit = move_schedule.begin();
       mit != move_schedule.end();
       mit = move_schedule.upper_bound(mit->first)) {
    mts++;
  }
...
bool GenLPFSSched::DetermineLeafFunction(Function *F) {
  ...
  for (vector<Instruction *>::reverse_iterator rit = InstList.rbegin();
       rit != InstList.rend(); ++rit) {
    isALeaf = analyzeIntrinsicCallInst(F, (*rit));
    if (!isALeaf) {
      if (dyn_cast<CallInst>((*rit))->getCalledFunction()->getName() !=
          "store_cbit")
        return false;
    }
  }
...
void GenLPFSSched::CountCriticalFunctionResources(Function *F) {
  ...
  for (vector<Instruction *>::reverse_iterator rit = callList.rbegin();
       rit != callList.rend(); ++rit)
...
bool GenLPFSSched::runOnModule(Module &M) {
  ...
  int timeStep = 0;
  errs() << "M: $::SIMD_K=" << RES_CONSTRAINT
         << "; $::SIMD_D=" << DATA_CONSTRAINT << "; $::SIMD_L=" << SIMD_L
         << "\n";
  ...
  for (scc_iterator<CallGraph *> sccIb = scc_begin(&CG);
    ...
    for (std::vector<CallGraphNode *>::const_iterator nsccI = nextSCC.begin(),
      ...
      if (F && !F->isDeclaration()) {
        ...
        vector<Function *>::iterator vit =
            find(isLeaf.begin(), isLeaf.end(), F);
        if (vit != isLeaf.end()) {
          errs() << "\nLPFS:\n";
          errs() << "Function: " << F->getName()
                 << " (sched: lpfs, k: " << RES_CONSTRAINT
                 << ", d: " << DATA_CONSTRAINT << " l: " << SIMD_L
                 << ", opp: " << OPP_SIMD << ", refill: " << REFILL << ") \n";
          errs() << "=========================================================="
                    "========\n";
