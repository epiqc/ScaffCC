...
struct qGateArg { // arguments to qgate calls
  ...
  int dimSize[MAX_QBIT_ARR_DIM]; // sizes of dimensions of array for qbit
                                 // declarations OR indices of specific qbit for
                                 // gate arguments
  int valOrIndex; // Value if not Qbit, Index if Qbit & not a Ptr
  double val;
  // Note: valOrIndex is of type integer. Assumes that quantities will be int in
  // the program.
  qGateArg()
      : argPtr(NULL), argNum(-1), isQbit(false), isCbit(false), isUndef(false),
        isPtr(false), isDouble(false), numDim(0), valOrIndex(-1), val(0.0) {}
};
...
struct DynGenQASMLoops : public ModulePass {
  ...
  std::map<Function *, vector<qGateArg> >
      qbitsInitInFunc; // new qbits declared in function
  std::map<Function *, vector<qGateArg> > funcArgList; // function arguments

  std::map<Function *, std::vector<FnCall> >
      mapFunction;                      // trace sequence of qgate calls
  ...
  void analyzeAllocInstExec(Function *F, Instruction *pinst,
                            AllocaInst *strAlloc);
  ...
  void insertCallToAllocQbitExec(AllocaInst *AI, int varSize,
                                 AllocaInst *strAlloc);
  void insertCallToAllocCbitExec(AllocaInst *AI, int varSize,
                                 AllocaInst *strAlloc);

  void insertCallToHeader(Function *F, Instruction *I, AllocaInst *strAlloc);
  void instrumentIntrinsicInst(CallInst *CI, int id,
                               map<unsigned, pair<string, bool> > nameOfQbit,
                               AllocaInst *strAlloc);
  void instrumentNonIntrinsicInst(CallInst *CI,
                                  map<unsigned, pair<string, bool> > nameOfQbit,
                                  AllocaInst *strAlloc);
...
static RegisterPass<DynGenQASMLoops>
    X("dyn-gen-qasm-with-loops",
      "Generate QASM output code with repeat statements");

void DynGenQASMLoops::insertCallToHeader(Function *F, Instruction *I,
                                         AllocaInst *strAlloc) {
  ...
  std::stringstream ss;
  ss << std::left << std::setw(MAX_FUNCTION_NAME - 1) << std::setfill('.')
     << fnName;
  Constant *StrConstant =
      ConstantDataArray::getString(I->getContext(), ss.str());
...
bool DynGenQASMLoops::backtraceOperand(Value *opd, int opOrIndex) {
  ...
  std::vector<Value *>::iterator vIter =
      std::find(vectQbit.begin(), vectQbit.end(), opd);
...
void DynGenQASMLoops::insertCallToAllocQbit(AllocaInst *AI, int varSize,
                                            AllocaInst *strAlloc) {
  ...
  idxVect.push_back(ConstantInt::get(Type::getInt32Ty(AI->getContext()), 0));
  GetElementPtrInst *arrPtr =
      GetElementPtrInst::Create(Type::getInt32Ty(AI->getContext()),
                                (Value *)AI, idxVect, "", FirstInst);
  ...
  ss << std::left << std::setw(MAX_FUNCTION_NAME - 1) << std::setfill('.')
     << AI->getName().str();
  Constant *StrConstant =
      ConstantDataArray::getString(AI->getContext(), ss.str());
  new StoreInst(StrConstant, strAlloc, "", FirstInst);
  Value *Idx[2];
  ...
  GetElementPtrInst *strPtr =
      GetElementPtrInst::Create(Type::getInt32Ty(AI->getContext()), strAlloc,
                                Idx, "", FirstInst);
...
void DynGenQASMLoops::insertCallToAllocCbit(AllocaInst *AI, int varSize,
                                            AllocaInst *strAlloc) {
...
void DynGenQASMLoops::insertCallToAllocQbitExec(AllocaInst *AI, int varSize,
                                                AllocaInst *strAlloc) {
...
void DynGenQASMLoops::insertCallToAllocCbitExec(AllocaInst *AI, int varSize,
                                                AllocaInst *strAlloc) {
...
void DynGenQASMLoops::instrumentIntrinsicInst(
    CallInst *CI, int id, map<unsigned, pair<string, bool> > nameOfQbit,
    AllocaInst *strAlloc) {
  ...
  for (std::map<unsigned, pair<string, bool> >::iterator iter =
           nameOfQbit.begin();
       iter != nameOfQbit.end(); ++iter) {
    ...
    ss << std::left << std::setw(MAX_FUNCTION_NAME - 1) << std::setfill('.')
       << qname;
    Constant *StrConstant =
        ConstantDataArray::getString(CI->getContext(), ss.str());
    ...
    GetElementPtrInst *strPtr =
        GetElementPtrInst::Create(Type::getInt32Ty(CI->getContext()), strAlloc,
                                  Idx, "", CI);
...
void DynGenQASMLoops::instrumentNonIntrinsicInst(
    CallInst *CI, map<unsigned, pair<string, bool> > nameOfQbit,
    AllocaInst *strAlloc) {
  std::stringstream ss;
  ss << std::left << std::setw(MAX_FUNCTION_NAME - 1) << std::setfill('.')
  ...
  GetElementPtrInst *strPtr = GetElementPtrInst::Create(Type::getInt32Ty(CI->getContext()), strAlloc, Idx, "", CI);
...
void DynGenQASMLoops::analyzeAllocInst(Function *F, Instruction *pInst,
                                       AllocaInst *strAlloc) {
    ...
    if (ArrayType *arrayType = dyn_cast<ArrayType>(allocatedType)) {
      ...
      */
      ...
      if (elementType->isIntegerTy(16)) {
        ...
        // find argName2 in funcArgList - avoid printing out qbit declaration
        // twice
        std::map<Function *, vector<qGateArg> >::iterator mIter =
            funcArgList.find(F);
        if (mIter != funcArgList.end()) {
          bool foundit = false;
          for (vector<qGateArg>::iterator vParamIter = (*mIter).second.begin();
               (vParamIter != (*mIter).second.end() && !foundit);
               ++vParamIter) {
...
void DynGenQASMLoops::analyzeAllocInstExec(Function *F, Instruction *pInst,
                                           AllocaInst *strAlloc) {
  if (AllocaInst *AI = dyn_cast<AllocaInst>(pInst)) {
    Type *allocatedType = AI->getAllocatedType();
...
void DynGenQASMLoops::analyzeCallInst(Function *F, Instruction *pInst,
                                      AllocaInst *strAlloc) {
  if (CallInst *CI = dyn_cast<CallInst>(pInst)) {
    ...
    if (CF->getName().find("qasm_print_") != string::npos)
      return;
    ...
    std::map<unsigned, double>
        valueOfDouble; // map argument index to const double value
    ...
    for (unsigned iop = 0; iop < CI->getNumArgOperands(); iop++) {
      if (ConstantInt *CInt = dyn_cast<ConstantInt>(CI->getArgOperand(iop)))
      ...
      else if (ConstantFP *CDouble =
                   dyn_cast<ConstantFP>(CI->getArgOperand(iop)))
        ...
        if (argType->isPointerTy()) {
          ...
          if (argElemType->isIntegerTy(16)) {
            ...
            nameOfQbit[iop] =
                make_pair(tmpDepQbit[0].argPtr->getName().str(), true);
          } else if (argElemType->isIntegerTy(1)) {
            backtraceOperand(CI->getArgOperand(iop), 0);
            assert(tmpDepQbit[0].argPtr != NULL &&
                   "Expected to find cbit ptr as function argument");
            nameOfQbit[iop] =
                make_pair(tmpDepQbit[0].argPtr->getName().str(), true);
          }
          ...
          nameOfQbit[iop] =
              make_pair(tmpDepQbit[0].argPtr->getName().str(), false);
    ...
    if (CallInst *CI = dyn_cast<CallInst>(pInst)) {
      if (CI->getCalledFunction()->getName() == "store_cbit") {
        vRemoveInst.push_back(CI);
                    ...
                    .find("qasm_print_RepLoopStart") != string::npos) ||
               ((CI->getCalledFunction()->getName())
                    .find("qasm_print_RepLoopEnd") != string::npos)) {
        vRemoveInst.push_back(CI);
      }
    }
  }
...
void DynGenQASMLoops::analyzeInst(Function *F, Instruction *pInst,
                                  AllocaInst *strAlloc) {
  if (debugDynGenQASMLoops)
...
void DynGenQASMLoops::printFuncHeader(Function *F) {
  ...
  if (tmp_num_elem > 0) {
    ...
    errs() << " "
           << printVarName(
                  ((*mpItr2).second[tmp_num_elem - 1]).argPtr->getName());
  }
  ...
  for (vector<qGateArg>::iterator vvit = (*mvpItr).second.begin(),
                                  vvitE = (*mvpItr).second.end();
       vvit != vvitE; ++vvit) {
    if ((*vvit).isQbit)
...
bool DynGenQASMLoops::runOnModule(Module &M) {
  ...
  qasmAllocQbit = cast<Function>(M.getOrInsertFunction(
      "qasm_print_qbit_alloc", Type::getVoidTy(M.getContext()),
      Type::getInt16Ty(M.getContext())->getPointerTo(),
      Type::getInt32Ty(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(), (Type *)0));

  qasmAllocCbit = cast<Function>(M.getOrInsertFunction(
      "qasm_print_cbit_alloc", Type::getVoidTy(M.getContext()),
      Type::getInt32Ty(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(), (Type *)0));

  qasmAllocQbitExec = cast<Function>(M.getOrInsertFunction(
      "qasm_print_qbit_alloc_exec", Type::getVoidTy(M.getContext()),
      Type::getInt16Ty(M.getContext())->getPointerTo(),
      Type::getInt32Ty(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(), (Type *)0));

  qasmAllocCbitExec = cast<Function>(M.getOrInsertFunction(
      "qasm_print_cbit_alloc_exec", Type::getVoidTy(M.getContext()),
      Type::getInt32Ty(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(), (Type *)0));

  qasmHeader = cast<Function>(M.getOrInsertFunction(
      "qasm_print_header", Type::getVoidTy(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(), (Type *)0));

  qasmHeaderQbit = cast<Function>(M.getOrInsertFunction(
      "qasm_print_header_qbit_arg", Type::getVoidTy(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(), (Type *)0));

  qasmHeaderQbitPtr = cast<Function>(M.getOrInsertFunction(
      "qasm_print_header_qbitptr_arg", Type::getVoidTy(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(), (Type *)0));

  qasmHeaderCbitPtr = cast<Function>(M.getOrInsertFunction(
      "qasm_print_header_cbitptr_arg", Type::getVoidTy(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(), (Type *)0));

  qasmHeaderEnd = cast<Function>(M.getOrInsertFunction(
      "qasm_print_header_end", Type::getVoidTy(M.getContext()),
      Type::getInt32Ty(M.getContext()), (Type *)0));

  qasmHeaderIntArg = cast<Function>(M.getOrInsertFunction(
      "qasm_print_header_int_arg", Type::getVoidTy(M.getContext()),
      Type::getInt32Ty(M.getContext()), (Type *)0));

  qasmHeaderDoubleArg = cast<Function>(M.getOrInsertFunction(
      "qasm_print_header_double_arg", Type::getVoidTy(M.getContext()),
      Type::getDoubleTy(M.getContext()), (Type *)0));

  qasmQCall = cast<Function>(M.getOrInsertFunction(
      "qasm_print_call_start", Type::getVoidTy(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(), (Type *)0));

  qasmQCallArgQbit = cast<Function>(M.getOrInsertFunction(
      "qasm_print_call_qbit_arg", Type::getVoidTy(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(),
      Type::getInt16Ty(M.getContext()), (Type *)0));

  qasmQCallArgQbitPtr = cast<Function>(M.getOrInsertFunction(
      "qasm_print_call_qbitptr_arg", Type::getVoidTy(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(), (Type *)0));

  qasmQCallEnd = cast<Function>(M.getOrInsertFunction(
      "qasm_print_call_end", Type::getVoidTy(M.getContext()),
      Type::getInt32Ty(M.getContext()), (Type *)0));

  qasmGate = cast<Function>(
      M.getOrInsertFunction("qasm_print_qgate", Type::getVoidTy(M.getContext()),
                            Type::getInt32Ty(M.getContext()),
                            Type::getInt8Ty(M.getContext())->getPointerTo(),
                            Type::getInt16Ty(M.getContext()), (Type *)0));

  qasmGate2 = cast<Function>(M.getOrInsertFunction(
      "qasm_print_qgate2", Type::getVoidTy(M.getContext()),
      Type::getInt32Ty(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(),
      Type::getInt8Ty(M.getContext())->getPointerTo(),
      Type::getInt16Ty(M.getContext()), Type::getInt16Ty(M.getContext()),
      (Type *)0));

  qasmGate3 = cast<Function>(M.getOrInsertFunction(
      "qasm_print_qgate3", Type::getVoidTy(M.getContext()),
      Type::getInt32Ty(M.getContext()),
      Type::getInt8Ty(M.getContext())->getPointerTo(),
      Type::getInt8Ty(M.getContext())->getPointerTo(),
      Type::getInt8Ty(M.getContext())->getPointerTo(),
  ...
  qasmRot = cast<Function>(
      M.getOrInsertFunction("qasm_print_rot", Type::getVoidTy(M.getContext()),
                            Type::getInt32Ty(M.getContext()),
                            Type::getInt8Ty(M.getContext())->getPointerTo(),
                            Type::getInt16Ty(M.getContext()),
                            Type::getDoubleTy(M.getContext()), (Type *)0));

  qasmCallInstIntArg = cast<Function>(M.getOrInsertFunction(
      "qasm_print_call_int_arg", Type::getVoidTy(M.getContext()),
      Type::getInt32Ty(M.getContext()), (Type *)0));

  qasmCallInstDoubleArg = cast<Function>(M.getOrInsertFunction(
      "qasm_print_call_double_arg", Type::getVoidTy(M.getContext()),
      Type::getDoubleTy(M.getContext()), (Type *)0));
  ...
  for (scc_iterator<CallGraph *> sccIb = scc_begin(&CG); !sccIb.isAtEnd(); ++sccIb) {
    ...
    for (std::vector<CallGraphNode *>::const_iterator nsccI = nextSCC.begin(),
      ...
      if (F && F->getName().find("_qtm") != string::npos)
        isQtmImage = true;
      ...
      if (F && !F->isDeclaration() && isQtmImage) {
        ...
        while (isa<AllocaInst>(*firstInstIter))
          ++firstInstIter;
        FirstInst = &(*firstInstIter);

        // insert alloca insts here
        ArrayType *strTy = ArrayType::get(
            Type::getInt8Ty(FirstInst->getContext()), MAX_FUNCTION_NAME);
        AllocaInst *strAlloc = new AllocaInst(strTy, AS, "", FirstInst);

        // insert call to print qasm header
        // insertCallToHeader(F->getName().str(),FirstInst);
        insertCallToHeader(F, FirstInst, strAlloc);

        // visit Alloc Insts in func and find if function is quantum or
        // classical function
        for (inst_iterator instIb = inst_begin(F), instIe = inst_end(F);
             instIb != instIe; ++instIb) {

          Instruction *pInst =
              &*instIb; // Grab pointer to instruction reference
      ...
      if (F && !F->isDeclaration() && !isQtmImage) {
        ...
        for (vector<Instruction *>::iterator iterInst = vRemoveInst.begin();
             iterInst != vRemoveInst.end(); ++iterInst) {
