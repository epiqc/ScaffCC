//===---------------- RuntimeFrequencyEstimationHybrid.cpp ----------------===//
//
...
struct RTFreqEstHyb : public ModulePass {
  ...
  void visitCallInst(BasicBlock::iterator I, AllocaInst *strAlloc,
                     AllocaInst *intArrAlloc, AllocaInst *doubleArrAlloc) {
    ...
    bool delAfterInst =
        true; // do not delete Meas gates because it will invalidate cbit stores
    ...
    if (CF->isIntrinsic()) {
      ...
      if (isIntrinsicQuantum) {
        ...
        Constant *gateID = ConstantInt::get(
            Type::getInt32Ty(CF->getContext()), gateIndex, false);
      ...
      // double *double_params, unsigned num_doubles, unsigned repeat)

      vector<Value *> vectCallArgs;

      std::stringstream ss;
      ss << std::left << std::setw(_MAX_FUNCTION_NAME - 1) << std::setfill(' ')
         << CF->getName().str();
      ...
      GetElementPtrInst *strPtr =
          GetElementPtrInst::Create(Type::getInt32Ty(CI->getContext()),
                                    strAlloc, Idx, "", (Instruction *)CI);
      ...
      for (unsigned iop = 0; iop < CI->getNumArgOperands(); iop++) {
        ...
        if (ConstantInt *CInt = dyn_cast<ConstantInt>(callArg)) {
          ...
          vIntArgs.push_back(intArgPtr);
                   ...
                   Type::getInt32Ty(
                       CI->getContext())) { // FIXME: make sure it's an integer
          intArgPtr = CastInst::CreateIntegerCast(
              callArg, Type::getInt32Ty(CI->getContext()), false, "",
              (Instruction *)CI);
          num_ints++;
          vIntArgs.push_back(intArgPtr);
        }
        ...
        if (ConstantFP *CDouble =
          ...
          doubleArgPtr = CastInst::CreateFPCast(
              callArg, Type::getDoubleTy(CI->getContext()), "",
              (Instruction *)CI);
          num_doubles++;
          vDoubleArgs.push_back(doubleArgPtr);
        }
      ...
      for (unsigned i = 0; i < num_ints; i++) {
        ...
        Value *intPtr = GetElementPtrInst::CreateInBounds(intArrAlloc, Idx, "",
                                                          (Instruction *)CI);
        new StoreInst(Int, intPtr, "", (Instruction *)CI);
      }
      Idx[1] = ConstantInt::get(Type::getInt32Ty(CI->getContext()), 0);
      GetElementPtrInst *intArrPtr = GetElementPtrInst::CreateInBounds(
          intArrAlloc, Idx, "", (Instruction *)CI);

      for (unsigned i = 0; i < num_doubles; i++) {
        Value *Double = vDoubleArgs[i];
        Idx[1] = ConstantInt::get(Type::getInt32Ty(CI->getContext()), i);
        Value *doublePtr = GetElementPtrInst::CreateInBounds(
            doubleArrAlloc, Idx, "", (Instruction *)CI);
        new StoreInst(Double, doublePtr, "", (Instruction *)CI);
      }
      GetElementPtrInst *doubleArrPtr = GetElementPtrInst::CreateInBounds(
          doubleArrAlloc, Idx, "", (Instruction *)CI);

      Constant *IntNumConstant = ConstantInt::get(
          Type::getInt32Ty(CF->getContext()), num_ints, false);
      Constant *DoubleNumConstant = ConstantInt::get(
          Type::getInt32Ty(CF->getContext()), num_doubles, false);
      ...
      rep_val =
          ConstantInt::get(Type::getInt32Ty(CF->getContext()), 1, false);
      vInstRemove.push_back((Instruction *)CI);
    }
  }
  ...
  void visitFunction(Function &F) {
    ...
    if (!F.isDeclaration() && isQuantumModule) {
      ...
      BasicBlock::iterator BBiter = BB_first->getFirstNonPHI()->getIterator();
      while (isa<AllocaInst>(BBiter))
        ++BBiter;
      Instruction *pInst = &(*BBiter);

      ArrayType *strTy = ArrayType::get(Type::getInt8Ty(pInst->getContext()),
                                        _MAX_FUNCTION_NAME);
      AllocaInst *strAlloc = new AllocaInst(strTy, AS, "", pInst);

      ArrayType *intArrTy = ArrayType::get(
          Type::getInt32Ty(pInst->getContext()), _MAX_INT_PARAMS);
      AllocaInst *intArrAlloc = new AllocaInst(intArrTy, AS, "", pInst);

      ArrayType *doubleArrTy = ArrayType::get(
          Type::getDoubleTy(pInst->getContext()), _MAX_DOUBLE_PARAMS);
      AllocaInst *doubleArrAlloc = new AllocaInst(doubleArrTy, AS, "", pInst);
  ...
  bool runOnModule(Module &M) {
    ...
    // void exit_scope ()
    exit_scope = cast<Function>(M.getOrInsertFunction(
        "exit_scope", Type::getVoidTy(M.getContext()), (Type *)0));

    // void initialize ()
    qasmInitialize = cast<Function>(M.getOrInsertFunction(
        "qasm_initialize", Type::getVoidTy(M.getContext()), (Type *)0));
    ...
    // void qasmGate ()
    qasmGate = cast<Function>(M.getOrInsertFunction(
        "qasm_gate", Type::getVoidTy(M.getContext()), (Type *)0));
    ...
    // iterate over instructions to instrument the initialize and exit scope
    // calls
    // insert alloca instructions at the beginning for subsequent memoize calls
...
static RegisterPass<RTFreqEstHyb> X("runtime-frequency-estimation-hybrid",
                                    "Estimate frequency of modules at runtime");
