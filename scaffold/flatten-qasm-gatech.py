...
def process_qasm(fname):
    ...
    for q in qgates_5: #MeasX, MeasZ
        ...
        #fstr = 'void '+instFnName+'(char* a, char* b){ printf("' +gateNames[q] +' %s\\n",a); }\n'
        fstr = 'void '+instFnName+'(char* a){ printf("' +gateNames[q] +' %s\\n",a); }\n'
        fout.write(fstr)
    ...
    while(b!=''):
        ...
        #pattern = re.compile(r"\s*\bqbit\b\s+(?P<qbit_var>\w+)\s*\[\s*(?P<array_size>\d+)\s*\]\s*;")
        ...
        if(m):
            #print 'Matched qbit_decl',b
            #print 'Var = ', m.group('qbit_var')
            #print 'Size = ', m.group('array_size')
                ...
                if qstr in qgates:
                    ...
                    if(m1):
                        ...
                        if(retStr):
                            ...
                            #print 'New retstr=',retStr
                            #argStr = m1.group('array_size')                        
                            #argReplStr = argStr+', '+retStr
                            #mystr = mystr.replace(argStr,argReplStr)
                        #print 'Meas modified'
                    #else:
                        #print 'No Meas'
            ...
            #arrInitStr = var+'['+str(numElem)+'] = {'

            #fout.write(arrInitStr)
