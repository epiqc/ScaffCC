...
../build/Release+Asserts/bin/clang -cc1 -O1 -emit-llvm qasm-resource-est.c -o qasm-resource-est.bc
...
../build/Release+Asserts/bin/llvm-link qasm-resource-est.bc ${fname}_compile.bc -S -o=${fname}_res_linked.bc

echo "Instrumenting ${fname}_res_linked.bc"
../build/Release+Asserts/bin/opt -load ../build/Release+Asserts/lib/Scaffold.dylib -runtime-resource-estimation ${fname}_res_linked.bc -o ${fname}_res_instr.bc
...
rm qasm-resource-est.bc ${fname}_compile.bc ${fname}_res_linked.bc ${fname}_res_instr.bc ${fname}_res_instr2.bc
