...
/*       <stdio.h>  is included in "bbrl________________sdll_sgt.h" */
/*       <stdint.h> is included in "bbrl________________sdll_sgt.h" */
#include <stdlib.h> /* `malloc' */



typedef struct {
  ptr_to_entry_of_sgt_ATOM p;
  int                         descend_to_the_left;
} path_element_for_sgt_insert;



ptr_to_el_of_sll_of_unused_el_of_heap_ATOM
  sll_of_unused_el_of_sdll_ATOM = NULL;
union abstract_el_of_heap_ATOM *
  temp_for_internal_use_ATOM;
struct elem_of_stack_of_allocated_blocks_ATOM *
  stack_of_allocated_blocks_ATOM = NULL;



/*
A small program for "bc" which generates
"min_num_elements_for_sgt_ATOM_of_given_height".
...

{
  ...
  print "  #define MAX_NUM_LEVELS_IN_SGT_ATOM ";
  i
  print "  int64_t min_num_elements_for_sgt_ATOM_of_given_height[\n"
  print "    MAX_NUM_LEVELS_IN_SGT_ATOM + 7] = {\n\n"
  for (i = 0; e(- l(alpha) * i) <= 2^63; i++) {
    i
    e(- l(alpha) * i) + 1 - 10^(-74)
    print "\n"
...
#define MAX_NUM_LEVELS_IN_SGT_ATOM 108
int64_t min_num_elements_for_sgt_ATOM_of_given_height[
  MAX_NUM_LEVELS_IN_SGT_ATOM + 7] = {
...
static ptr_to_entry_of_sgt_ATOM
  stack_for_subtree_rebuild[MAX_NUM_LEVELS_IN_SGT_ATOM + 7];
static ptr_to_entry_of_sgt_ATOM *
...
static ptr_to_entry_of_sgt_ATOM
  NULL_ptr_to_entry_of_sgt_ATOM_having_an_address = NULL;
...
print_to_file_ATOM(
FILE * f, ATOM * arg) {
  fprintf(f, "The first byte of ATOM is 0x%02X",
    (int) *((unsigned char *) arg));
  if ( (*((unsigned char *) arg) >= 32)
       &&
       (*((unsigned char *) arg) <= 126) ) {
    fprintf(f, " '%c'", *((unsigned char *) arg));
  }
}
...
void) {
  ...
  if (is_the_first_call != 0) {
    ...
    if (sizeof(struct node_of_sdll_sgt_ATOM)
        !=
        sizeof(union abstract_el_of_heap_ATOM)) {
      fprintf(stderr,
              "WARNING : sdll_sgt.c\n");
      fprintf(stderr,
              "sizeof(struct node_of_sdll_sgt_ATOM) = %d.\n",
              (int) sizeof(struct node_of_sdll_sgt_ATOM));
      fprintf(stderr,
              "sizeof(union abstract_el_of_heap_ATOM) = %d.\n",
              (int) sizeof(union abstract_el_of_heap_ATOM));
      fprintf(stderr,
              ...
              "`new_block_of_99_elem_of_sdll_ATOM' to ignore "
              "this warning.\n");
      exit(-1);
    }

    if (sizeof(struct s_block_ATOM)
        !=
        sizeof(union abstract_el_of_heap_ATOM)) {
      fprintf(stderr,
              "WARNING : sdll_sgt.c\n");
      fprintf(stderr,
              "sizeof(struct s_block_ATOM) = %d.\n",
              (int) sizeof(struct s_block_ATOM));
      fprintf(stderr,
              "sizeof(union abstract_el_of_heap_ATOM) = %d.\n",
              (int) sizeof(union abstract_el_of_heap_ATOM));
      fprintf(stderr,
  ...
  ptr_to_new_block =
    (struct elem_of_stack_of_allocated_blocks_ATOM *)
    malloc(sizeof(struct elem_of_stack_of_allocated_blocks_ATOM));
  ...
  update_memory_usage_info(
    &heap_size_ATOM,
    sizeof(struct elem_of_stack_of_allocated_blocks_ATOM)
  );

  ptr_to_new_block->ptr_to_next_elem_of_stack =
    stack_of_allocated_blocks_ATOM;
  ...
  /* --- BEGIN add all new el. to deleted_el_of_sdll_ATOM --- */
  for (i = 99 - 2; i >= 0; i--) {
  ...
  /* ---  END  add all new el. to deleted_el_of_sdll_ATOM --- */
}
...
void) {
  ...
  struct elem_of_stack_of_allocated_blocks_ATOM * stack_it =
    stack_of_allocated_blocks_ATOM;
  ptr_to_el_of_sll_of_unused_el_of_heap_ATOM iter =
    sll_of_unused_el_of_sdll_ATOM;
  while (stack_it != NULL) {
    num_alloc_blocks++;
...
reset_heap_ATOM(
void) {
  struct elem_of_stack_of_allocated_blocks_ATOM * it;
  while(stack_of_allocated_blocks_ATOM != NULL) {
    it = stack_of_allocated_blocks_ATOM;
    stack_of_allocated_blocks_ATOM =
      stack_of_allocated_blocks_ATOM->ptr_to_next_elem_of_stack;
    free(it);
  }
  sll_of_unused_el_of_sdll_ATOM = NULL;
}
...
FILE * f, sdll_ATOM sdll) {
  iter_sdll_ATOM h;
  set_iter_to_leftmost_el_sdll_ATOM(h, sdll)
  while (h != NULL) {
    print_to_file_ATOM(f, &(h->e.body));
    printf("\n");
    move_iter_right_sdll_ATOM(h)
  }
}
...
print_sdll_ATOM(
sdll_ATOM sdll) {
  print_to_file_sdll_ATOM(stdout, sdll);
}
...
str_on_heap_ATOM * target, str_on_heap_ATOM source) {
  ...
  while ( (*(write_head++) = *(read_head++)) != '\0' ) {
    if (--i == 0) {
      if (sll_of_unused_el_of_sdll_ATOM == NULL)
        new_block_of_99_elem_of_sdll_ATOM();
      last_b->s.ptr_to_next_s_block = sll_of_unused_el_of_sdll_ATOM;
      sll_of_unused_el_of_sdll_ATOM =
        sll_of_unused_el_of_sdll_ATOM->u.ptr_to_next_el_of_sll;
      last_b = last_b->s.ptr_to_next_s_block;
      write_head = &(last_b->s.body[0]);
...
str_on_heap_ATOM arg) {
  str_on_heap_ATOM temp;
  while(arg != NULL) {
    temp = arg->s.ptr_to_next_s_block;
    arg->u.ptr_to_next_el_of_sll = sll_of_unused_el_of_sdll_ATOM;
    sll_of_unused_el_of_sdll_ATOM = arg;
    arg = temp;
  }
}
...
str_on_heap_ATOM arg) {
  ...
  while (arg->s.ptr_to_next_s_block != NULL) {
    ...
    rv += (int64_t)
          (sizeof(struct node_of_sdll_sgt_ATOM)
          -
          sizeof(str_on_heap_ATOM));
  }
...
str_on_heap_ATOM arg1, str_on_heap_ATOM arg2) {
  ...
  if (arg1 == arg2) { /* Compare as pointers to resolve strcat with self. */
    ...
    last_b = arg1;
    write_i = sizeof(struct node_of_sdll_sgt_ATOM)
              -
              sizeof(str_on_heap_ATOM);
    ...
    read_i = sizeof(struct node_of_sdll_sgt_ATOM)
             -
             sizeof(str_on_heap_ATOM);
...
str_on_heap_ATOM * target) {
  return h_ATOM_alloc_fscan_str(stdin, target);
}
...
FILE * stream, str_on_heap_ATOM s, int64_t ml, char pad, char just) {
  ...
  int64_t string_length = h_ATOM_strlen(s);
  ...
  switch (just) {
    ...
    default:
      fprintf(stderr,
              "Error! Function h_ATOM_fprintf_str(...) "
...
str_on_heap_ATOM s, int64_t ml, char pad, char just) {
  return h_ATOM_fprintf_str(stdout, s, ml, pad, just);
}
...
sdll_ATOM * sdll) {
  ptr_to_node_of_sdll_sgt_ATOM   left_rh;
  ptr_to_node_of_sdll_sgt_ATOM   right_rh;
  ptr_to_node_of_sdll_sgt_ATOM   left_block_end;
  ptr_to_node_of_sdll_sgt_ATOM   right_block_end;
  ptr_to_node_of_sdll_sgt_ATOM   ptr_to_first_el_of_sll;
  ptr_to_node_of_sdll_sgt_ATOM * ptr_to_last_ptr_to_right_of_sll;
  ptr_to_node_of_sdll_sgt_ATOM * ptr_to_ptr_to_next_block;
  ptr_to_node_of_sdll_sgt_ATOM   ptr_to_last_el_inserted;



  set_iter_to_leftmost_el_sdll_ATOM(left_rh, *sdll)
  ...
  do { /* while (1). Terminates by `break'. */
    ...
    *ptr_to_last_ptr_to_right_of_sll = NULL;
    ptr_to_last_ptr_to_right_of_sll = &ptr_to_first_el_of_sll;
    ptr_to_ptr_to_next_block = &temp_for_internal_use_ATOM;
  ...
  */
  ...
  if (sll_of_unused_el_of_sdll_ATOM == NULL)
    new_block_of_99_elem_of_sdll_ATOM();
  ptr_to_last_el_inserted = sll_of_unused_el_of_sdll_ATOM;
  sll_of_unused_el_of_sdll_ATOM =
    sll_of_unused_el_of_sdll_ATOM->u.ptr_to_next_el_of_sll;
...
static ptr_to_node_of_sdll_sgt_ATOM
build_2balanced_tree_from_sll(
int64_t n) {
  /* `temp_for_internal_use_ATOM' is the read head.            */
  /* The argument is always >= 2 (including all recursive calls). */

  ptr_to_node_of_sdll_sgt_ATOM rv;
  ptr_to_node_of_sdll_sgt_ATOM t;
  int64_t left_branch_n;

  switch (n) {

    case 2:
    rv = temp_for_internal_use_ATOM;
    rv->e.ptr_to_right = t = temp_for_internal_use_ATOM =
      temp_for_internal_use_ATOM->e.ptr_to_right;
    ...
    temp_for_internal_use_ATOM =
      temp_for_internal_use_ATOM->e.ptr_to_right;
    rv->e.ptr_to_left = t;
    t = rv->e.ptr_to_right = temp_for_internal_use_ATOM;
    temp_for_internal_use_ATOM =
      temp_for_internal_use_ATOM->e.ptr_to_right;
    t->e.ptr_to_left = t->e.ptr_to_right = NULL;
    return rv;
    ...
    t = t->e.ptr_to_right = temp_for_internal_use_ATOM;
    temp_for_internal_use_ATOM =
      temp_for_internal_use_ATOM->e.ptr_to_right;
    t->e.ptr_to_left = t->e.ptr_to_right = NULL;
    return rv;
...
sgt_ATOM * sgt, sdll_ATOM * sdll) {
  ...
  *ptr_to_last_ptr_to_right_of_sll = NULL;
  ...
  /* `temp_for_internal_use_ATOM' to make it accessible for  */
  /* the recursive function `build_2balanced_tree_from_sll'.    */
  ptr_to_last_ptr_to_right_of_sll = &temp_for_internal_use_ATOM;
  ...
  /* sll starts at `temp_for_internal_use_ATOM'           */
...
ptr_to_entry_of_sgt_ATOM p) {
  if (p == NULL) return 0;
    ...
    return weight_of_ATOM(p->e.ptr_to_left)
           +
           weight_of_ATOM(p->e.ptr_to_right)
           +
           1;
  }
}
...
ptr_to_entry_of_sgt_ATOM ptr_e, sgt_ATOM * sgt) {
  ...
  int     h;
  path_element_for_sgt_insert
    path_to_new_node[MAX_NUM_LEVELS_IN_SGT_ATOM + 7];
  path_element_for_sgt_insert * path_end;
  int depth_of_new_node = 0;
  ptr_to_entry_of_sgt_ATOM pn = sgt->ptr_to_root;

  if (pn == NULL) {
    sgt->ptr_to_root = ptr_e;
  } else while (1) {
    depth_of_new_node++;
    if (ATOM_arg1_GREATER_THAN_arg2(ptr_e->e.body, pn->e.body)) {
  ...
  if (sgt->num_entries
      <
      min_num_elements_for_sgt_ATOM_of_given_height[depth_of_new_node]) {
    ...
    do {
      ...
      if (path_end->descend_to_the_left != 0) {
        w = w + 1 + weight_of_ATOM(path_end->p->e.ptr_to_right);
      } else {
        w = w + 1 + weight_of_ATOM(path_end->p->e.ptr_to_left);
      }
    } while (w >= min_num_elements_for_sgt_ATOM_of_given_height[h]);
...
ptr_to_entry_of_sgt_ATOM ptr_e, sgt_ATOM * sgt) {
  ...
  if (pn == NULL) {
    ...
    depth_of_new_node++;
    if (ATOM_arg1_GREATER_THAN_arg2(pn->e.body, ptr_e->e.body)) {
      if (pn->e.ptr_to_left != NULL) {
        pn = pn->e.ptr_to_left;
      } else {
        pn->e.ptr_to_left = ptr_e;
...
ATOM * key, sgt_ATOM * sgt) {
  ATOM *                     rv = NULL;
  ptr_to_entry_of_sgt_ATOM   temp = sgt->ptr_to_root;
  ptr_to_entry_of_sgt_ATOM * p = &(sgt->ptr_to_root);

  ptr_to_home_sgt_of_the_element_ready_for_rm = sgt;

  while (temp != NULL)
  if ( ATOM_arg1_GREATER_THAN_arg2( (*key), (temp->e.body)) ) {
    p =  &(temp->e.ptr_to_right);
    temp = temp->e.ptr_to_right;
  } else if ( ATOM_arg1_GREATER_THAN_arg2( (temp->e.body), (*key)) ) {
  ...
  if (rv == NULL) ptr_to_ptr_to_the_element_ready_for_rm =
    &NULL_ptr_to_entry_of_sgt_ATOM_having_an_address;
...
void) {
  ptr_to_entry_of_sgt_ATOM * p;
  ptr_to_entry_of_sgt_ATOM   temp;
  ptr_to_entry_of_sgt_ATOM   rv;
...
ptr_to_entry_of_sgt_ATOM
for_debug_only__get_el_prep_rm_ATOM(
void) {
...
int64_t
for_debug_only__min_w_required_ATOM(
int h) {
  return min_num_elements_for_sgt_ATOM_of_given_height[h];
}
